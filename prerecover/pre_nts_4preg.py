...
def parse_args():
    ...
    args.preg_csv = r'../data/recover/output/pregnancy_CX_0306/pregnancy_{}.csv'.format(args.dataset)
    args.output_file = r'../data/recover/output/pregnancy_CX_0306/pregnancy_{}_nts.csv'.format(args.dataset)
    if args.dataset == 'montefiore':
        args.preg_csv = r'../data/recover/output/pregnancy_CX_0306/pregnancy_montefiore.csv'
        args.output_file = r'../data/recover/output/pregnancy_CX_0306/pregnancy_montefiore_nts.csv'
...
def read_nts(dataset, output_file, selected_patients={}):
    ...
    dfs_hf_all.to_csv(output_file+'.gz', index=False, compression='gzip')
...
if __name__ == '__main__':
    ...
    #
    #
    df = pd.read_csv(args.preg_csv, dtype={'patid': str, 'site': str, 'zip': str},
                     parse_dates=['index date', 'dob',
                                  'flag_delivery_date',
                                  'flag_pregnancy_start_date',
                                  'flag_pregnancy_end_date'])
    print('df.shape', df.shape)
    selected_patients = df.loc[(df['flag_pregnancy'] == 1) | (df['flag_exclusion'] == 1), 'patid'].to_list()
    print('len(selected_patients)', len(selected_patients), 'len(set(selected_patients))', len(set(selected_patients)))

    df = read_nts(args.dataset, args.output_file, set(selected_patients))
