...
def parse_args():
    ...
    parser.add_argument('--cohorts', choices=['covid_posneg18base', 'covid_posOnly18base',
                                              'covid_posnegpreg', 'covid_posnegpreg-negInpos'],
                        default='covid_posnegpreg', help='cohorts')
...
def _load_mapping():
    ...
    # add preg related drug, rought combine
    preg_med = utils.load(r'../data/mapping/pregnant_drug_grt.pkl')
    ...
    return (icd_pasc, pasc_encoding, icd_cmr, cmr_encoding, icd_ccsr, ccsr_encoding,
            ...
            icd_addedPaxRisk, addedPaxRisk_encoding, icd_negctrlpasc, negctrlpasc_encoding, ssri_med, preg_med
            )
...
def _encoding_bmi_and_smoking(vital_list, index_date):
    ...
    # 2024-7-23 a simple outlier preprocess method. some wt / ht are wrong
    if pd.notna(bmi) and ((bmi > 80) or (bmi < 5)):
        bmi = bmi_select
...
def build_feature_matrix(args):
    ...
    (icd_pasc, pasc_encoding, icd_cmr, cmr_encoding,
     ...
     fips_ziplist, icd_CFR, CFR_encoding, icd_addedPaxRisk, addedPaxRisk_encoding,
     icd_negctrlpasc, negctrlpasc_encoding, ssri_med, preg_med) = _load_mapping()
    ...
    for site in tqdm(sites):
        ...
        # cohorts_preg_roughcal_wcm_pcornet_all.pkl.gz
        ...
        output_dx_info = r'../data/recover/output/{}/info_dx_preg_roughcal_{}_{}.csv'.format(
            args.dataset, args.cohorts, args.dataset)
        ...
        for pid, item in tqdm(id_data.items(), total=len(id_data), mininterval=10):
            # for i, (pid, item) in tqdm(enumerate(id_data.items()), total=len(id_data)):
            index_info, demo, dx_raw, med, covid_lab, encounter, procedure, obsgen, immun, death, vital, id_lab_select, id_pregnant = item
            flag, preg_or_tup, index_date, covid_loinc, flag_name, index_age, index_enc_id, index_enc_type, index_source = index_info
            if len(demo) == 0:
            ...
            site_list = []
            covid_list = []
            preg_or_tup_list = [] # preg related records indicator, or tup for pos contributed neg portions
            ...
            bmi_list = []
            yearmonth_array = np.zeros((n, 46), dtype='int16')
            ...
            index_period_array = np.zeros((n, 12), dtype='int16')
            ...
            # 2025-1-22 pregnant related drug
            ...
            column_names = ['patid', 'site', 'covid', 'preg_or_tup', 'index date', 'hospitalized',
                           ...
                           ssritreat_column_names + pregtreat_column_names  # + outcome_med_column_names
            # if args.positive_only:
            #     if not flag:
            #         continue
            # i += 1
            ...
            preg_or_tup_list.append(preg_or_tup)
            ...
            cci_array[i, :] = _encoding_cci_and_score(dx_raw, icd_cci, cci_encoding, index_date)
            ...
            ###
            # 2024-4-3 and 2024-7-23  capture ssri and snri for exploration
            ...
            #
            # 2025-1-22 preg treat related drug, no time contraints
            pregtreat_flag[i, :], pregtreat_t2e[i, :], pregtreat_t2eall_1row = \
                _encoding_covidtreat(med, pregtreat_names, preg_med, index_date, default_t2e)
            pregtreat_t2eall.append(pregtreat_t2eall_1row)
