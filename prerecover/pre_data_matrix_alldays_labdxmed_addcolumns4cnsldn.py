...
def build_feature_matrix(args):
    ...
    (icd_pasc, pasc_encoding, icd_cmr, cmr_encoding,
     ...
     cnsldn_med, icd_covCNSLDN, covCNSLDN_encoding) = _load_mapping()  # no load fips_ziplist
    ...
    for site in tqdm(sites):
        ...
        # 2025-4-8 add LDN CNS to old cohort for preliminary analysis
        output_file_query12_bool = r'../data/recover/output/{}/matrix_cohorts_{}-nbaseout-alldays-preg_{}-addCFR-PaxRisk-U099-Hospital-SSRI-v7-CNSLDN.csv'.format(
            args.dataset, args.cohorts, args.dataset)
        ...
        for pid, item in tqdm(id_data.items(), total=len(id_data), mininterval=10):
            ...
            column_names = (['patid', 'site', 'covid', ] + outcome_CFR_column_names + addPaxRisk_column_names +
                            ...
                            cnsldntreat_column_names + outcome_covCNSLDN_column_names +
            ...
            outcome_covCNSLDN_flag[i, :], outcome_covCNSLDN_t2e[i, :], outcome_covCNSLDN_baseline[i, :], outcome_covCNSLDN_t2eall_1row = \
                _encoding_outcome_dx_withalldaysoveralltime(dx, icd_covCNSLDN, covCNSLDN_encoding, index_date, default_t2e)
