...
def shell_lab_dx_med_4covid_addcolumnes25Q2():
    # python pre_codemapping.py 2>&1 | tee  log/pre_codemapping_zip_adi.txt
    start_time = time.time()
    ...
    with open(r'shell_all_2025Q2AddCulum.ps1', 'wt') as f:
        for i, site in enumerate(site_list):
            site = site.strip()

            cmdstr = """python pre_data_matrix_alldays_labdxmed_addcolumn25Q2.py --dataset nyu 2>&1 | tee  log\pre_data_matrix_alldays_labdxmed_addcolumn25Q2_nyu.txt
""".replace('nyu', site)
    ...
    utils.split_shell_file_bydelta(r"shell_all_2025Q2AddCulum.ps1", delta=ndelta, skip_first=0)
