...
def _load_mapping():
    ...
    # add ADHD drugs 2025-05-20
    adhd_ctrl_med = utils.load(r'../data/mapping/ADHD_ctrl_drugs_mapping.pkl')

    # add naltrexone related covs, 2025-07-11
    icd_covNaltrexone_multimap = utils.load(r'../data/mapping/icd_covNaltrexone_mapping.pkl')
    covNaltrexone_encoding = utils.load(r'../data/mapping/covNaltrexone_index_mapping.pkl')

    # add naltrexone drugs for covs 2025-07-11
    covNaltrexone_med = utils.load(r'../data/mapping/Naltrexone_drug_cov_mapping.pkl')
    ...
    return (icd_pasc, pasc_encoding, icd_cmr, cmr_encoding, icd_ccsr, ccsr_encoding,
            ...
            icd_mecfs, mecfs_encoding, icd_cvddeath, cvddeath_encoding, adhd_ctrl_med,
            icd_covNaltrexone_multimap, covNaltrexone_encoding, covNaltrexone_med)
...
def build_feature_matrix(args):
    ...
    (icd_pasc, pasc_encoding, icd_cmr, cmr_encoding,
     ...
     adhd_ctrl_med,
     icd_covNaltrexone_multimap, covNaltrexone_encoding, covNaltrexone_med) = _load_mapping()  # no load fips_ziplist
    ...
    for site in tqdm(sites):
        ...
        id_data = utils.load(input_file, num_cpu=8)
        ...
        for pid, item in tqdm(id_data.items(), total=len(id_data), mininterval=10):
            ...
            # 2025-5-20  capture adhdctrl for active comparator
            adhdctrl_flag[i, :], adhdctrl_t2e[i, :], adhdctrl_t2eall_1row = \
        ...
        print('Done! Dump data bool matrix for 25Q2 to {}'.format(output_file_query12_bool))
