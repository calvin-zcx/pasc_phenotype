...
def plot_heatmap_for_dx_subgroup_absCumIncidence_split_V3(database='V15_COVID19', type='cif', month=6, pasc_flg=True,
    ...
    for i, organ in enumerate(organ_list):
        ...
        for key, row in df_select.iterrows():
            ...
            if (database == 'oneflorida') and (row['selected'] == 1):
                name += r'$^{‡}$'

            if (type == 'hr') or (type == 'hr-w-p'):
                hr = row['hr-w']
                ci = stringlist_2_list(row['hr-w-CI'])
                p = row['hr-w-p']
            elif type == 'km':
            ...
            domain = row['Organ Domain']
            if pasc == 'PASC-General':
                pasc_row = [name, hr, ci, p, domain, key, hrp]
                continue
            if domain == organ:
                organ_n[i] += 1
                if len(name.split()) >= 5:
    ...
    if pasc_flg:
        ...
        pval.append(pasc_row[3])
        key_list.append(pasc_row[5])
        p_val_hr.append(pasc_row[6])

    # load other

    p_value_map = {'all': p_val_hr}

    if (type == 'cifdiff-pvalue') or (type == 'hr-w-p'):
        heat_value = {'all': pval, }
    else:
        heat_value = {'all': measure, }
    ...
    for severity in ['outpatient',
        ...
        _df = pd.read_csv(
            r'../data/{}/output/character/outcome/DX-{}-new-trim/causal_effects_specific.csv'.format(
                database, severity)).set_index('i')
        _df_selected = _df.loc[key_list, :]
        if type == 'hr':
            _hr = _df_selected.loc[:, 'hr-w']
            ...
            _hr = _df_selected.loc[:, 'hr-w-p']
        elif type == 'casepos':
            _hr = _df_selected.loc[:, 'no. pasc in +']

        heat_value[severity] = _hr.tolist()
        p_value_map[severity] = _df_selected.loc[:, 'hr-w-p'].tolist()
    ...
    df_heat_value = pd.DataFrame(heat_value)
    df_hrp = pd.DataFrame(p_value_map)
    df_hrp_bool = df_hrp <= 0.05 / 596
    ...
    if type == 'cifdiff-pvalue':
        # norm = LogNorm(vmin=data.min().min(), vmax=data.max().max())
        norm = LogNorm(vmin=0.05 / 596, vmax=1)
        fmt = ".1g"
        figw = 15  # 8
        figh = figw * asp
    elif type == 'hr-w-p':
        norm = LogNorm(vmin=0.05 / 596, vmax=1, clip=True)
        fmt = ".1g"
        figw = 15  # 8
        figh = figw * asp
        # cmap = "Blues_r"
    ...
    # axes[1, 2].set_xticklabels([9, 10, 11])
    if highlightcell:
        for ith, interval in enumerate([(0, 1), (1, 3), (3, 5), (5, 7), (7, 9), (9, 17)]):
            left, right = interval
            print(ith, left, right)
            ax = axes[ith]
            df_sub = df_hrp_bool.iloc[:, left:right]
            for irow in range(df_sub.shape[0]):
                for jcol in range(df_sub.shape[1]):
                    if df_sub.iloc[irow, jcol]:
                        ax.add_patch(
                            Rectangle((jcol, irow), 1, 1, fill=False, edgecolor='crimson', lw=2, alpha=0.7))  # crimson
    ...
    output_dir = r'../data/{}/output/character/outcome/figure/{}/'.format(database, type)
    check_and_mkdir(output_dir)
    plt.savefig(output_dir + 'new-trim-subgroup_heatmap_{}-{}-month{}{}-p{:.3f}-V3-MORE.png'.format(
        database, type, month, select_criteria, pvalue), bbox_inches='tight', dpi=600)
    plt.savefig(output_dir + 'new-trim-subgroup_heatmap_{}-{}-month{}{}-p{:.3f}-V3-MORE.pdf'.format(
        database, type, month, select_criteria, pvalue), bbox_inches='tight', transparent=True)
...
def plot_heatmap_for_dx_subgroup_absCumIncidence_split_timeperiod(database='V15_COVID19', type='cif', month=6,
    ...
    data = df_heat_value
    asp = data.shape[0] / float(data.shape[1]) / 1.8
    figw = 7  # 8
    figh = figw * asp
...
def plot_heatmap_for_dx_subgroup_absCumIncidence_split_timeperiod_Variant_V2(database='V15_COVID19', type='cif',
                                                                             month=6,
                                                                             pasc=True,
                                                                             star=False, select_criteria='',
                                                                             pvalue=0.05 / 137):
    ...
    if database == 'oneflorida':
        df = pd.read_excel(
            r'../data/oneflorida/output/character/outcome/DX-all-new-trim/causal_effects_specific_dx_oneflorida-MultiPval-DXMEDALL.xlsx', sheet_name='dx').set_index('i')
    elif database == 'V15_COVID19':
        df = pd.read_excel(
            r'../data/V15_COVID19/output/character/outcome/DX-all-new-trim/causal_effects_specific_dx_insight-MultiPval-DXMEDALL.xlsx',
            sheet_name='dx').set_index('i')
    ...
    if select_criteria == 'insight':
        print('select_critera:', select_criteria)
        _df = pd.read_excel(
            r'../data/V15_COVID19/output/character/outcome/DX-all-new-trim/causal_effects_specific_dx_oneflorida-MultiPval-DXMEDALL.xlsx',
            sheet_name='dx').set_index('i')
        # pvalue = 0.05 / 137
        _df_select = _df.sort_values(by='hr-w', ascending=False)
        # _df_select = _df_select.loc[_df_select['hr-w-p'] <= pvalue, :]  #
    ...
    for severity in ['1stwave', 'alpha', 'delta']:
        if database in ['oneflorida', 'V15_COVID19']:
            _df = pd.read_csv(
                r'../data/{}/output/character/outcome/DX-{}-new-trim/causal_effects_specific.csv'.format(
                    database, severity)).set_index('i')
        # elif database == 'pooled':
    ...
    df_heat_value = pd.DataFrame(heat_value).rename(columns={'all':'Overall', '1stwave':'Ancestral', 'alpha': 'Alpha & others', 'delta': 'Delta'})
    df_hrp = pd.DataFrame(p_value_map).rename(columns={'all':'Overall', '1stwave':'Ancestral', 'alpha': 'Alpha & others', 'delta': 'Delta'})
    df_hrp_bool = df_hrp <= 0.05 / 596
    ...
    if True:
        for ith, interval in enumerate([(0, 1), (1, 4),]):
            left, right = interval
            print(ith, left, right)
            ax = axes[ith]
            df_sub = df_hrp_bool.iloc[:, left:right]
    ...
    check_and_mkdir(output_dir)
    plt.savefig(output_dir + 'new-trim-subgroup_heatmap_{}-{}-variant{}{}-p{:.3f}.png'.format(
        database, type, month, select_criteria, pvalue), bbox_inches='tight', dpi=700)
    plt.savefig(output_dir + 'new-trim-subgroup_heatmap_{}-{}-variant{}{}-p{:.3f}.pdf'.format(
        database, type, month, select_criteria, pvalue), bbox_inches='tight', transparent=True)
