...
if __name__ == "__main__":
    ...
    if args.site == 'all':
        ...
        sites = ['wcm', 'montefiore', 'mshs', 'nyu', 'columbia']
        # sites = ['wcm', ]
        # sites = ['pitt', ]
    ...
    print('args.site:', args.site, 'len(sites), sites:', len(sites), sites)
    ...
    for ith, site in tqdm(enumerate(sites), total=len(sites)):
        ...
        data_file = r'../data/recover/output/pregnancy_CX_0306/pregnancy_{}.csv'.format(site)
    ...
    # pregnant patients only
    print('building eligible covid infection during pregnancy, and pregnant but with ectopic/abortions/etc not expected delivery outcome, might be combined with group1')
    print('Before selecting, len(df)', len(df))
    df = df.loc[(df['flag_pregnancy'] == 1) | (df['flag_exclusion'] == 1), :]
    print('After selecting pregnant females, len(df1)', len(df))
    ...
    df = feature_process_additional(df)
    utils.check_and_mkdir(r'../data/recover/output/pregnancy_output/')
    df.to_csv(r'../data/recover/output/pregnancy_output/pregnant_covidboth_normalornot-{}.csv'.format(args.site))
