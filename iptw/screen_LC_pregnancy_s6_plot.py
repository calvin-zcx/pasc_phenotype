...
def plot_forest_for_LC_pregnant_outcome_primary_v2_N3C(indir,  show='full', excludenull=False):
    ...
    selected_rows =  [
        ...
        'preterm birth<37Andlivebirth',
         ...
         'outcome_3_preeclampsia',
        'outcome_5_preeclampsia_sipec',
        'outcome_8_preeclampsia_spec',
        'outcome_9_preeclampsia_hellp',
        'outcome_7_preeclampsia_mpec',
        'outcome_10_preeclampsia_unpec',
        'outcome_6_preeclampsia_ghtn',
    ...
    'outcome_6_preeclampsia_ghtn': 'gHTN (O13)',
        'outcome_4_eclampsia': 'Eclampsia (O15)',
...
def plot_forest_for_dx_organ_ssri_lib2_cifdiff_primary_v3_individualSSRI(indir, drugname, ctrlname='Non-SSRI',
    ...
    axs = fp.forestplot(
        ...
        xticks=[0.1, 1, 2.5],
        # [0.5, 1, 2.],  #[0.1, 1, 10], #[0.3, 1, 2.5], #[0.5, 1, 2.],  # x-ticks to be printed  [0.5, 1, 35],  #
        color_alt_rows=True,
        # flush=True,
        # sort=False, #True,  # sort estimates in ascending order
        # sortby='-aHR',
    ...
    axs.axvline(x=1, ymin=0, ymax=0.95, color='grey', linestyle='dashed')
    check_and_mkdir(output_dir)
    plt.savefig(output_dir + '{}-hr_moretabs-{}-nosort-{}.png'.format(drugname, show, drugname), bbox_inches='tight',
                dpi=600)
    plt.savefig(output_dir + '{}-hr_moretabs-{}-nosort-{}.pdf'.format(drugname, show, drugname), bbox_inches='tight',
                transparent=True)
...
def plot_forest_for_SSRI_subgroup_lib2_cifdiff_pascoutcome(pasc_outcome, show='full', ):
    ...
    plt.savefig(output_dir + 'hr_subgroup-{}-{}-v2.png'.format(_clean_name(pasc_outcome), show), bbox_inches='tight',
                dpi=600)
    plt.savefig(output_dir + 'hr_subgroup-{}-{}-v2.pdf'.format(_clean_name(pasc_outcome), show), bbox_inches='tight',
                transparent=True)
    ...
    for i, organ in enumerate(organ_list):
        ...
        for key, row in df_select.iterrows():
            ...
            result = [name, pasc, organ_mapname[organ],
                      ...
                      cif_diff_boost,
                      '{:.2f} ({:.2f}, {:.2f})'.format(cif_diff_boost, cif_diff_ci_boost[0], cif_diff_ci_boost[1]),
                      cif_diff_p_boost,
