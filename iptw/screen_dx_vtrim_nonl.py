...
if __name__ == "__main__":
    ...
    for i, pasc in tqdm(enumerate(pasc_encoding.keys(), start=1), total=len(pasc_encoding)):
        ...
        model = ml.PropensityEstimator(learner='LIGHTGBM', paras_grid={
            'max_depth': [3, 4, 5],
            'learning_rate': np.arange(0.01, 1, 0.25),
            'num_leaves': np.arange(5, 50, 15),
            'min_child_samples': [100, 200, 300],
            'random_state': [args.random_seed], }).cross_validation_fit(
            covs_array, covid_label, verbose=0)
        ...
        out_file_balance = r'../data/{}/output/character/outcome/DX-{}{}-new-trim-nonlinear/{}-{}-results.csv'.format(
            args.dataset,
            args.severity,
        ...
        df_summary.to_csv(
            '../data/{}/output/character/outcome/DX-{}{}-new-trim-nonlinear/{}-{}-evaluation_balance.csv'.format(
                args.dataset,
                args.severity,
                '-select' if args.selectpasc else '',
                i, pasc))
        ...
        dfps.to_csv(
            '../data/{}/output/character/outcome/DX-{}{}-new-trim-nonlinear/{}-{}-evaluation_ps-iptw.csv'.format(
                args.dataset,
                args.severity,
                '-select' if args.selectpasc else '',
                i, pasc))
        try:
            figout = r'../data/{}/output/character/outcome/DX-{}{}-new-trim-nonlinear/{}-{}-PS.png'.format(
                args.dataset,
                args.severity,
                '-select' if args.selectpasc else '',
                i, pasc)
        ...
        km, km_w, cox, cox_w, cif, cif_w = weighted_KM_HR(
            ...
            fig_outfile=r'../data/{}/output/character/outcome/DX-{}{}-new-trim-nonlinear/{}-{}-km.png'.format(
                args.dataset,
                args.severity,
                '-select' if args.selectpasc else '',
                i, pasc),
        ...
        try:
            ...
            if i % 50 == 0:
                pd.DataFrame(causal_results, columns=results_columns_name). \
                    to_csv(
                    r'../data/{}/output/character/outcome/DX-{}{}-new-trim-nonlinear/causal_effects_specific-snapshot-{}.csv'.format(
            ...
            df_causal.to_csv(
                r'../data/{}/output/character/outcome/DX-{}{}-new-trim-nonlinear/causal_effects_specific-ERRORSAVE.csv'.format(
                    args.dataset,
                    args.severity,
                    '-select' if args.selectpasc else '', ))
    ...
    df_causal.to_csv(
        r'../data/{}/output/character/outcome/DX-{}{}-new-trim-nonlinear/causal_effects_specific.csv'.format(
            args.dataset,
            args.severity,
            '-select' if args.selectpasc else ''))
