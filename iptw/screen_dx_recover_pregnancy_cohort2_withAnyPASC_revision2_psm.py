...
def parse_args():
    ...
    parser.add_argument('--buildcohort', action='store_true')
...
if __name__ == "__main__":
    ...
    # %% Step 1. Load  Data and add CoVs, outcomes
    print('In cohorts_characterization_build_data...')
    if args.buildcohort:
        print('In building cohort from individual sites')
        if args.site == 'all':
        ...
        df_outcome_cols = ['death', 'death t2e'] + [x for x in
                                                    list(df.columns)
                                                    if x.startswith('dx') or x.startswith('smm') or x.startswith(
                'any_pasc')
                                                    ]
        ...
        # 2024-4-1 R2
        # add psm cohort here
        df.to_csv(r'../data/recover/output/results_before_20230903/pregnancy_data/pregnancy_ecm_round2.csv')
    else:
        infile = r'../data/recover/output/results_before_20230903/pregnancy_data/pregnancy_ecm_round2.csv'
        print('In Loading cohort from previously dumped file', infile)
        df = pd.read_csv(infile,
                         dtype={'patid': str, 'site': str, 'zip': str},
                         parse_dates=['index date', 'flag_delivery_date', 'flag_pregnancy_start_date',
                                      'flag_pregnancy_end_date'])
        print('df.shape:', df.shape)
        # the following setup directly copied from the above
        df_info = df[['patid', 'site', 'index date', 'hospitalized',
                      'ventilation', 'criticalcare', 'maxfollowup', 'death', 'death t2e',
                      'flag_pregnancy', 'flag_delivery_date', 'flag_pregnancy_start_date',
                      'flag_pregnancy_gestational_age', 'flag_pregnancy_end_date', 'flag_maternal_age',
    ...
    model = ml.PropensityEstimator(learner='LR', paras_grid={
        ...
        df_covs, df_label, verbose=0)

    ps = model.predict_ps(df_covs)
    ps_logit = model.predict_logit(df_covs)

    model.report_stats()
    iptw = model.predict_inverse_weight(df_covs, df_label, stabilized=True, clip=True)
    ...
    caliper = np.std(ps_logit) * 0.25

    print('\nCaliper (radius) is: {:.4f}\n'.format(caliper))

    knn = NearestNeighbors(n_neighbors=10, p=2, radius=caliper)
    knn.fit(ps_logit.reshape(-1, 1))
    ## Common support distances and indexes, not only for treated, but also for control
    distances, indexes = knn.kneighbors(ps_logit.reshape(-1, 1), n_neighbors=10)


    # this match works for treated only, att. can also change for ate also, match for whole
    def perfom_matching_att(row, indexes, df_data):
        ...
        for idx in indexes[current_index, :]:
            if current_index != idx:
                if (row.exposed == 1) and (df_data.loc[idx].exposed == 0):
                    # for treated, match control
                    return int(idx)
                # elif (row.exposed == 0) and (df_data.loc[idx].exposed == 1):
                #     # for control, match treated
                #     return int(idx)


    # df['matched_element'] = df.reset_index().apply(perfom_matching_att, axis=1, args=(indexes, df))

    df = df.reset_index() # add colum with column name 'index'
    df['matched_element'] = np.nan
    for index, rows in tqdm(df.iterrows(), total=df.shape[0]):
        df.loc[index, 'matched_element'] = perfom_matching_att(rows, indexes, df)
    # df['matched_element'] = df['matched_element'].astype('int64')
    ...
    for i, pasc in tqdm(enumerate(pasc_encoding.keys(), start=1), total=len(pasc_encoding)):
        ...
        # 2024-4-1 1:1 PS match

        matched_index = df.loc[covid_label[covid_label == 1].index, 'matched_element']

        # sampled_neg_index = covid_label[(covid_label == 0)].sample(n=int(args.negative_ratio * n_covid_pos),
        #                                                            replace=False, random_state=args.random_seed).index

        sampled_neg_index = matched_index.astype('int64')

        pos_neg_selected = pd.Series(False, index=pasc_baseline.index)
        pos_neg_selected.loc[sampled_neg_index] = True
        pos_neg_selected.loc[covid_label[covid_label == 1].index] = True
        #
        ...
        print(i, pasc, '-- Selected cohorts {}/{} ({:.2f}%), covid pos:neg = {}:{} sample ratio -/+={}, '
            ...
            covid_label.sum(), len(sampled_neg_index), '1:1 PSM',
        ...
        out_file_balance = r'../data/recover/output/results-20230825/DX-{}{}-Rev2PSM1to1/{}-{}-results.csv'.format(
            args.severity,
        ...
        df_summary.to_csv(
            '../data/recover/output/results-20230825/DX-{}{}-Rev2PSM1to1/{}-{}-evaluation_balance.csv'.format(
                args.severity,
                '-select' if args.selectpasc else '',
        ...
        dfps.to_csv(
            '../data/recover/output/results-20230825/DX-{}{}-Rev2PSM1to1/{}-{}-evaluation_ps-iptw.csv'.format(
                args.severity,
                '-select' if args.selectpasc else '',
                i, pasc.replace(':', '-').replace('/', '-')))
        try:
            figout = r'../data/recover/output/results-20230825/DX-{}{}-Rev2PSM1to1/{}-{}-PS.png'.format(
                args.severity,
                '-select' if args.selectpasc else '',
                i, pasc.replace(':', '-').replace('/', '-'))
            print('Dump ', figout)
        ...
        km, km_w, cox, cox_w, cif, cif_w = weighted_KM_HR(
            ...
            fig_outfile=r'../data/recover/output/results-20230825/DX-{}{}-Rev2PSM1to1/{}-{}-km.png'.format(
                args.severity,
                '-select' if args.selectpasc else '',
                i, pasc.replace(':', '-').replace('/', '-')),
            title=pasc,
        ...
        try:
            ...
            if i % 2 == 0:
                pd.DataFrame(causal_results, columns=results_columns_name). \
                    to_csv(
                    r'../data/recover/output/results-20230825/DX-{}{}-Rev2PSM1to1/causal_effects_specific-snapshot-{}.csv'.format(
            ...
            df_causal.to_csv(
                r'../data/recover/output/results-20230825/DX-{}{}-Rev2PSM1to1/causal_effects_specific-ERRORSAVE.csv'.format(
                    args.severity,
                    '-select' if args.selectpasc else '', ))
    ...
    df_causal.to_csv(
        r'../data/recover/output/results-20230825/DX-{}{}-Rev2PSM1to1/causal_effects_specific.csv'.format(
            args.severity,
            '-select' if args.selectpasc else ''))
