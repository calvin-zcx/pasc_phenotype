...
def plot_forest_for_dx_organ_preg_lib2(show='full'):
    ...
    # df = pd.read_csv(indir + 'causal_effects_specific.csv')
    df = pd.read_excel(indir + 'causal_effects_specific-edit.xlsx')
    df.drop_duplicates(subset=['pasc'], keep='last', inplace=True, )

    df = df.loc[df['selected_narrow_25'] == 1, :]
...
if __name__ == '__main__':
    ...
    # plot_forest_for_preg_subgroup_lib2_cifdiff(show='full')
    # plot_forest_for_preg_subgroup_lib2_cifdiff(show='short')
