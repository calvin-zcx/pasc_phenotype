...
def parse_args():
    ...
    # Input
    parser.add_argument('--dataset', choices=['pooled'], default='pooled',
                        help='data bases')  # 'oneflorida', 'V15_COVID19',
    ...
    # More args
    args.data_file = r'../data/V15_COVID19/output/character/matrix_cohorts_covid_4manuNegNoCovid_bool_ALL.csv'
    # .format(
    # args.dataset,
    # args.site)

    args.data_file2 = r'../data/oneflorida/output/character/matrix_cohorts_covid_4manuNegNoCovid_bool_all.csv'
...
if __name__ == "__main__":
    # python screen_dx_pooled.py --dataset pooled --severity icu 2>&1 | tee  log/screen_dx_pooled_icu.txt
    # python screen_dx_pooled.py --dataset pooled --severity inpatient 2>&1 | tee  log/screen_dx_pooled_inpatient.txt
    # python screen_dx_pooled.py --dataset pooled --severity outpatient 2>&1 | tee  log/screen_dx_pooled_outpatient.txt
    # python screen_dx_pooled.py --dataset pooled --severity all 2>&1 | tee  log/screen_dx_pooled_all.txt
    ...
    for i, pasc in tqdm(enumerate(pasc_encoding.keys(), start=1), total=len(pasc_encoding)):
        ...
        database = df_database[pos_neg_selected]  # newly added for interaction terms
        ...
        out_file_balance = r'../data/V15_COVID19/output/character/outcome/{}/DX-{}{}/{}-{}-results.csv'.format(
            args.dataset,
            args.severity,
        ...
        df_summary.to_csv(
            '../data/V15_COVID19/output/character/outcome/{}/DX-{}{}/{}-{}-evaluation_balance.csv'.format(
                args.dataset,
                args.severity,
                '-select' if args.selectpasc else '',
                i, pasc))

        km, km_w, cox, cox_w, cox_inter, cif, cif_w = weighted_KM_HR_pooled(
            covid_label, iptw, pasc_flag, pasc_t2e, database,
            fig_outfile=r'../data/V15_COVID19/output/character/outcome/{}/DX-{}{}/{}-{}-km.png'.format(
                args.dataset,
                args.severity,
                '-select' if args.selectpasc else '',
                i, pasc),
