...
if __name__ == "__main__":
    ...
    for i, pasc in tqdm(enumerate(drug_name_list, start=1), total=len(drug_name_list)):
        ...
        out_file_balance = r'../data/{}/output/character/outcome/MED-{}-new-trim-vaccine/{}-{}-results.csv'.format(args.dataset,
                                                                                                      args.severity, i,
                                                                                                      pasc)
        ...
        df_summary.to_csv(
            r'../data/{}/output/character/outcome/MED-{}-new-trim-vaccine/{}-{}-evaluation_balance.csv'.format(args.dataset,
                                                                                                  args.severity, i,
                                                                                                  pasc))
        dfps = pd.DataFrame({'ps': ps, 'iptw': iptw, 'covid': covid_label})

        dfps.to_csv(
            r'../data/{}/output/character/outcome/MED-{}-new-trim-vaccine/{}-{}-evaluation_ps-iptw.csv'.format(args.dataset,
                                                                                                  args.severity, i,
                                                                                                  pasc))
        try:
            figout = r'../data/{}/output/character/outcome/MED-{}-new-trim-vaccine/{}-{}-{}-PS.png'.format(
        ...
        try:
            km, km_w, cox, cox_w, cif, cif_w = weighted_KM_HR(
                ...
                fig_outfile=r'../data/{}/output/character/outcome/MED-{}-new-trim-vaccine/{}-{}-{}-km.png'.format(
                    args.dataset,
                    args.severity, i,
                    pasc, drugname),
                title=pasc + '-' + drugname)
        ...
        try:
            ...
            if i % 50 == 0:
                pd.DataFrame(causal_results, columns=results_columns_name). \
                    to_csv(
                    r'../data/{}/output/character/outcome/MED-{}-new-trim-vaccine/causal_effects_specific_med-snapshot-{}.csv'.format(
            ...
            df_causal.to_csv(
                r'../data/{}/output/character/outcome/MED-{}-new-trim-vaccine/causal_effects_specific_med-ERRORSAVE.csv'.format(
                    args.dataset, args.severity))
    ...
    df_causal.to_csv(r'../data/{}/output/character/outcome/MED-{}-new-trim-vaccine/causal_effects_specific_med.csv'.format(
        args.dataset, args.severity))
