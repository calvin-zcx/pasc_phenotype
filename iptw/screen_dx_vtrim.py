...
if __name__ == "__main__":
    ...
    for i, pasc in tqdm(enumerate(pasc_encoding.keys(), start=1), total=len(pasc_encoding)):
        ...
        out_file_balance = r'../data/{}/output/character/outcome/DX-{}{}-new-trim/{}-{}-results.csv'.format(
            args.dataset,
            args.severity,
        ...
        df_summary.to_csv(
            '../data/{}/output/character/outcome/DX-{}{}-new-trim/{}-{}-evaluation_balance.csv'.format(
                args.dataset,
                args.severity,
                '-select' if args.selectpasc else '',
                i, pasc))
        ...
        dfps.to_csv(
            '../data/{}/output/character/outcome/DX-{}{}-new-trim/{}-{}-evaluation_ps-iptw.csv'.format(
                args.dataset,
                args.severity,
                '-select' if args.selectpasc else '',
                i, pasc))
        try:
            figout = r'../data/{}/output/character/outcome/DX-{}{}-new-trim/{}-{}-PS.png'.format(
                args.dataset,
                args.severity,
                '-select' if args.selectpasc else '',
                i, pasc)
        ...
        km, km_w, cox, cox_w, cif, cif_w = weighted_KM_HR(
            ...
            fig_outfile=r'../data/{}/output/character/outcome/DX-{}{}-new-trim/{}-{}-km.png'.format(
                args.dataset,
                args.severity,
                '-select' if args.selectpasc else '',
                i, pasc),
        ...
        try:
            ...
            if i % 50 == 0:
                pd.DataFrame(causal_results, columns=results_columns_name). \
                    to_csv(r'../data/{}/output/character/outcome/DX-{}{}-new-trim/causal_effects_specific-snapshot-{}.csv'.format(
            ...
            df_causal.to_csv(
                r'../data/{}/output/character/outcome/DX-{}{}-new-trim/causal_effects_specific-ERRORSAVE.csv'.format(
                    args.dataset,
                    args.severity,
                    '-select' if args.selectpasc else '',))
