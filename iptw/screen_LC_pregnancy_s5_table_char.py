...
def build_exposure_group_and_table1_print_explore_V2():
    # %% Step 1. Load  Data
    start_time = time.time()
    np.random.seed(0)
    random.seed(0)
    ...
    args = parse_args()
    in_file_df = r'../data/recover/output/pregnancy_output_y4/pregnant_yr4_exposureBuilt-pregafter180-all-anypasc.csv'
    out_file_df = in_file_df.replace('.csv', '-CharacterTableV2.xlsx')
    ...
    col_names_out = ([
                ...
                'Immunocompromised condition or weakened immune system',
                'Obesity',
                'Anemia, Sickle cell  or thalassemia',
                'Smoking current',
                'Stroke or cerebrovascular disease',
                'Substance use disorders',  # use this one, more than obc: substance one
                'Cancer',
                'Tuberculosis'] +
            [
...
if __name__ == '__main__':
    ...
    # 2025-11-7
    df, df_out = build_exposure_group_and_table1_print_explore_V2()
