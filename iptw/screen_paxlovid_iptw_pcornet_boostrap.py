...
from misc.causal_contrast import st_survival_difference_at_fixed_point_withBoostrapResults
...
if __name__ == "__main__":
    ...
    if args.cohorttype == 'atrisk':
        print('selecting cohorttype atrisk Further combine df3 and df4')
        print('fname3:', fname3)
        print('fname4:', fname4)
    ...
    for i, pasc in tqdm(enumerate(selected_screen_list, start=1), total=len(selected_screen_list)):
        ...
        out_file_balance = r'../data/recover/output/results/Paxlovid-{}-{}-{}-boostrap/{}-{}-results.csv'.format(
            args.cohorttype,
        ...
        df_summary.to_csv(
            '../data/recover/output/results/Paxlovid-{}-{}-{}-boostrap/{}-{}-evaluation_balance.csv'.format(
                args.cohorttype,
                args.severity.replace(':', '_').replace('/', '-').replace(' ', '_'),
                'pcornet',  # '-select' if args.selectpasc else '',
                i, _clean_name_(pasc)))
        ...
        dfps.to_csv(
            '../data/recover/output/results/Paxlovid-{}-{}-{}-boostrap/{}-{}-evaluation_ps-iptw.csv'.format(
                args.cohorttype,
                args.severity.replace(':', '_').replace('/', '-').replace(' ', '_'),
                'pcornet',  # '-select' if args.selectpasc else '',
                i, _clean_name_(pasc)))
        try:
            figout = r'../data/recover/output/results/Paxlovid-{}-{}-{}-boostrap/{}-{}-PS.png'.format(
                args.cohorttype,
                args.severity.replace(':', '_').replace('/', '-').replace(' ', '_'),
                'pcornet',  # '-select' if args.selectpasc else '',
                i, _clean_name_(pasc))
        ...
        df_boostrap = pd.DataFrame({'index': df_label[pos_neg_selected].index,
                                    'ps': ps,
                                    'iptw': iptw,
                                    'treated': covid_label,
                                    'event': pasc_flag,
        ...
        result_list = weighted_CIF_boostrap(df_boostrap, ntimes=1000,
                                            fig_outfile=r'../data/recover/output/results/Paxlovid-{}-{}-{}-boostrap/{}-{}-km.png'.format(
                                                args.cohorttype,
                                                args.severity.replace(':', '_').replace('/', '-').replace(' ', '_'),
                                                'pcornet',  # '-select' if args.selectpasc else '',
                                                i, _clean_name_(pasc)),
        ...
        alpha = 0.05
        cif_diff_list = [x[2][-1] for x in result_list]
        cif1_list = [x[0][-1] for x in result_list]
        cif0_list = [x[1][-1] for x in result_list]
        diff_quantile_ci = np.percentile(cif_diff_list, q=(100 * alpha / 2, 100 * (1 - alpha / 2)))
        cif1_quantile_ci = np.percentile(cif1_list, q=(100 * alpha / 2, 100 * (1 - alpha / 2)))
        cif0_quantile_ci = np.percentile(cif0_list, q=(100 * alpha / 2, 100 * (1 - alpha / 2)))
        ...
        try:
            _results = [i, pasc,
                        ...
                        test_result.diff_of_mean, np.mean(cif1_list), np.mean(cif0_list),
                        cif1_quantile_ci[0], cif1_quantile_ci[1], cif0_quantile_ci[0], cif0_quantile_ci[1],
                        np.mean(cif_diff_list), diff_quantile_ci[0], diff_quantile_ci[1], test_result.p_value,
                        test_result.diff_of_mean_lower, test_result.diff_of_mean_upper,
            ...
            results_columns_name = [
                ...
                'cif-w-diff-CILowerCardinal', 'cif-w-diff-CIUpperCardinal',
                'best_hyper_paras']
            print('causal result:\n', causal_results[-1])
            ...
            if i % 2 == 0:
                pd.DataFrame(causal_results, columns=results_columns_name). \
                    to_csv(
                    r'../data/recover/output/results/Paxlovid-{}-{}-{}-boostrap/causal_effects_specific-snapshot-{}.csv'.format(
                        args.cohorttype,
                        args.severity.replace(':', '_').replace('/', '-').replace(' ', '_'),
                        'pcornet',  # '-select' if args.selectpasc else '',
                        i))
            ...
            df_causal.to_csv(
                r'../data/recover/output/results/Paxlovid-{}-{}-{}-boostrap/causal_effects_specific-ERRORSAVE.csv'.format(
                    args.cohorttype,
                    args.severity.replace(':', '_').replace('/', '-').replace(' ', '_'),
                    'pcornet',  # '-select' if args.selectpasc else '',
                ))
