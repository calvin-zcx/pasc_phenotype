...
if __name__ == "__main__":
    ...
    # downsample to oneflorida size
    print('before downsample df.shape:', df.shape)
    _label = df['covid']
    df_pos_sample = df.loc[_label==1, :].sample(n=22341, replace=False, random_state=args.random_seed)
    df_neg_sample = df.loc[_label==0, :].sample(n=177010, replace=False, random_state=args.random_seed)
    df = pd.concat([df_pos_sample, df_neg_sample])
    print('after downsample df.shape:', df.shape)
    ...
    for i, pasc in tqdm(enumerate(pasc_encoding.keys(), start=1), total=len(pasc_encoding)):
        ...
        out_file_balance = r'../data/{}/output/character/outcome/DX-{}{}-new-trim-downsample/{}-{}-results.csv'.format(
            args.dataset,
            args.severity,
        ...
        df_summary.to_csv(
            '../data/{}/output/character/outcome/DX-{}{}-new-trim-downsample/{}-{}-evaluation_balance.csv'.format(
                args.dataset,
                args.severity,
                '-select' if args.selectpasc else '',
                i, pasc))
        ...
        dfps.to_csv(
            '../data/{}/output/character/outcome/DX-{}{}-new-trim-downsample/{}-{}-evaluation_ps-iptw.csv'.format(
                args.dataset,
                args.severity,
                '-select' if args.selectpasc else '',
                i, pasc))
        try:
            figout = r'../data/{}/output/character/outcome/DX-{}{}-new-trim-downsample/{}-{}-PS.png'.format(
                args.dataset,
                args.severity,
                '-select' if args.selectpasc else '',
                i, pasc)
        ...
        km, km_w, cox, cox_w, cif, cif_w = weighted_KM_HR(
            ...
            fig_outfile=r'../data/{}/output/character/outcome/DX-{}{}-new-trim-downsample/{}-{}-km.png'.format(
                args.dataset,
                args.severity,
                '-select' if args.selectpasc else '',
                i, pasc),
        ...
        try:
            ...
            if i % 50 == 0:
                pd.DataFrame(causal_results, columns=results_columns_name). \
                    to_csv(r'../data/{}/output/character/outcome/DX-{}{}-new-trim-downsample/causal_effects_specific-snapshot-{}.csv'.format(
            ...
            df_causal.to_csv(
                r'../data/{}/output/character/outcome/DX-{}{}-new-trim-downsample/causal_effects_specific-ERRORSAVE.csv'.format(
                    args.dataset,
                    args.severity,
                    '-select' if args.selectpasc else '',))
    ...
    df_causal.to_csv(
        r'../data/{}/output/character/outcome/DX-{}{}-new-trim-downsample/causal_effects_specific.csv'.format(
            args.dataset,
            args.severity,
            '-select' if args.selectpasc else ''))
