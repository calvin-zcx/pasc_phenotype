...
def add_col(df):
    ...
    # added 2025-7-14, more inclusive Obesity, bmi >= 30, no overweight
    df["PaxRisk:Obesity"] = (
            (df["DX: Severe Obesity  (BMI>=40 kg/m2)"] >= 1) | (df['bmi'] >= 30) | (df['addPaxRisk:Obesity'] >= 1) | (df['dxcovNaltrexone-base@Obesity'] >= 1)
    ).astype('int')
...
if __name__ == "__main__":
    ...
    if args.build_data:
        ...
        for ith, site in tqdm(enumerate(sites)):
            ...
            data_file = r'../data/recover/output/{}/matrix_cohorts_covid_posOnly18base-nbaseout-alldays-25Q2_{}.csv'.format(
                site, site)
            ...
            # add new columns 2, add later, waiting for ZX's running
            data_file_add = r'../data/recover/output/pregnancy_CX_0501_2025/pregnancy_{}.csv'.format(site)
        ...
        print('dump {} done!'.format(out_data_file),
    ...
    # else:
        ...
        # added 2024-7-14 Q3
        # comment out: just use as cov, not as exposure, so no need
        ...
        for x in cnsldn_names:
            ...
            df['cnsldn-treat--60-30@' + x] = 0
            df['cnsldn-treat-0-90@' + x] = 0
            df['cnsldn-treat--365--60@' + x] = 0
            df['cnsldn-treat--1095--60@' + x] = 0
            ...
            for x in cnsldn_names:
                ...
                if pd.notna(t2eall):
                    ...
                    for t2e in t2eall:
                        ...
                        if -60 <= t2e < 30:
                            df.loc[index, 'cnsldn-treat--60-30@' + x] = 1
                        if 0 <= t2e < 90:
                            df.loc[index, 'cnsldn-treat-0-90@' + x] = 1
                        if -365 <= t2e < -60:
                            df.loc[index, 'cnsldn-treat--365--60@' + x] = 1
                        if -1095 <= t2e < -60:
                            df.loc[index, 'cnsldn-treat--1095--60@' + x] = 1
        ...
        for x in cnsldn_names:
            ...
            print('cnsldn-treat--60-30@' + x, df['cnsldn-treat--60-30@' + x].sum())
            print('cnsldn-treat-0-90@' + x, df['cnsldn-treat-0-90@' + x].sum())
