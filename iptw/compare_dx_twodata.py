...
if __name__ == "__main__":
    ...
    df1 = df1.loc[df1['covid'] == 1, :]
    df1['covid'] = 1  # data 1 as covid 1 as cases

    df2 = pd.read_csv(args.data_file2, dtype={'patid': str}, parse_dates=['index date'])
    df2 = df2.loc[df2['covid'] == 1, :]
    df2['covid'] = 0  # data 1 as covid 0 as controls
    ...
    for i, pasc in tqdm(enumerate(pasc_encoding.keys(), start=1), total=len(pasc_encoding)):
        ...
        out_file_balance = r'../data/{}/output/character/outcome/DX2Com-{}{}/{}-{}-results.csv'.format(
            args.dataset,
            args.severity,
        ...
        df_summary.to_csv(
            '../data/{}/output/character/outcome/DX2Com-{}{}/{}-{}-evaluation_balance.csv'.format(
                args.dataset,
                args.severity,
                '-select' if args.selectpasc else '',
                i, pasc))
        ...
        km, km_w, cox, cox_w, cif, cif_w = weighted_KM_HR(
            ...
            fig_outfile=r'../data/{}/output/character/outcome/DX2Com-{}{}/{}-{}-km.png'.format(
                args.dataset,
                args.severity,
                '-select' if args.selectpasc else '',
                i, pasc),
        ...
        try:
            ...
            if i % 50 == 0:
                pd.DataFrame(causal_results, columns=results_columns_name). \
                    to_csv(r'../data/{}/output/character/outcome/DX2Com-{}{}/causal_effects_specific-snapshot-{}.csv'.format(
            ...
            df_causal.to_csv(
                r'../data/{}/output/character/outcome/DX2Com-{}{}/causal_effects_specific-ERRORSAVE.csv'.format(
                    args.dataset,
                    args.severity,
                    '-select' if args.selectpasc else '',))
    ...
    df_causal.to_csv(
        r'../data/{}/output/character/outcome/DX2Com-{}{}/causal_effects_specific.csv'.format(
            args.dataset,
            args.severity,
            '-select' if args.selectpasc else ''))
