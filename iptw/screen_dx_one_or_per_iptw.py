...
if __name__ == "__main__":
    # python screen_dx_one_or_per_iptw.py --dataset ALL 2>&1 | tee  log/screen_dx_one_or_per_iptw.txt
    ...
    # one weights for all, check balance
    model = ml.PropensityEstimator(learner='LR', random_seed=args.random_seed).cross_validation_fit(df_covs,
                                                                                                    df_label,
                                                                                                    verbose=0)
    ...
    out_file_balance = r'../data/V15_COVID19/output/character/outcome/one_or_per_iptw/{}-weights-for-all-results.csv'.\
        format(args.dataset)
    utils.check_and_mkdir(out_file_balance)
    model.results.to_csv(out_file_balance)  # args.save_model_filename +

    df_summary = summary_covariate(df_covs, df_label, iptw, smd, smd_weighted, before, after)
    df_summary.to_csv(r'../data/V15_COVID19/output/character/outcome/one_or_per_iptw/{}-weights-for-all-balance-results.csv'.format(args.dataset))
    ...
    for i, pasc in tqdm(enumerate(pasc_encoding.keys(), start=1)):
        ...
        # out_file_balance = r'../data/V15_COVID19/output/character/outcome/one_or_per_iptw/{}-{}-results.csv'.format(i, pasc)
        # utils.check_and_mkdir(out_file_balance)
        # model.results.to_csv(out_file_balance)  # args.save_model_filename +

        # df_summary = summary_covariate(covs_array, covid_label, iptw, smd, smd_weighted, before, after)
        # df_summary.to_csv('../data/V15_COVID19/output/character/outcome/one_or_per_iptw/{}-{}-evaluation_balance.csv'.format(i, pasc))
        ...
        try:
            ...
            if i % 50 == 0:
                pd.DataFrame(causal_results, columns=[
                    ...
                    to_csv(r'../data/V15_COVID19/output/character/outcome/one_or_per_iptw/causal_effects_specific-snapshot-{}.csv'.format(i))
        except:
            ...
            df_causal.to_csv(r'../data/V15_COVID19/output/character/outcome/one_or_per_iptw/causal_effects_specific-ERRORSAVE.csv')
    ...
    df_causal.to_csv(r'../data/V15_COVID19/output/character/outcome/one_or_per_iptw/causal_effects_specific.csv')
