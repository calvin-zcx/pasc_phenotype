...
if __name__ == "__main__":
    ...
    if args.cohorttype == 'atrisknopreg':
        ...
        fname1 = r'recover29Nov27_covid_pos_addCFR-PaxRisk-U099-Hospital-Preg_4PCORNetPax-negctrl-addPaxFeats-treated-noRisk-220301-230201.csv'
        fname2 = r'recover29Nov27_covid_pos_addCFR-PaxRisk-U099-Hospital-Preg_4PCORNetPax-negctrl-addPaxFeats-ctrl-noRisk-220301-230201.csv'
    ...
    select_codes = ['FAC003', 'FAC006', 'FAC008']
    select_group_name = 'Neoplasm encounter'
    negctrlpasc_list.append(select_group_name)
    group_dx_col(df, select_codes, select_group_name, negctrlpasc_encoding.keys())

    select_codes = ['NEO{:03d}'.format(x) for x in range(1, 75)]
    select_group_name = 'NEO 001-074'
    negctrlpasc_list.append(select_group_name)
    group_dx_col(df, select_codes, select_group_name, negctrlpasc_encoding.keys())
    ...
    select_codes = ['NEO{:03d}'.format(x) for x in range(12, 22)]
    select_group_name = 'Gastrointestinal cancers'
    negctrlpasc_list.append(select_group_name)
    group_dx_col(df, select_codes, select_group_name, negctrlpasc_encoding.keys())

    select_codes = ['NEO{:03d}'.format(x) for x in range(23, 29)]
    select_group_name = 'Skin cancers'
    negctrlpasc_list.append(select_group_name)
    group_dx_col(df, select_codes, select_group_name, negctrlpasc_encoding.keys())

    select_codes = ['NEO{:03d}'.format(x) for x in range(31, 39)]
    select_group_name = 'Female cancers'
    negctrlpasc_list.append(select_group_name)
    group_dx_col(df, select_codes, select_group_name, negctrlpasc_encoding.keys())

    select_codes = ['NEO{:03d}'.format(x) for x in range(39, 43)]
    select_group_name = 'Male reproductive system cancers'
    negctrlpasc_list.append(select_group_name)
    group_dx_col(df, select_codes, select_group_name, negctrlpasc_encoding.keys())

    select_codes = ['NEO{:03d}'.format(x) for x in range(43, 48)]
    select_group_name = 'Urinary system cancers'
    negctrlpasc_list.append(select_group_name)
    group_dx_col(df, select_codes, select_group_name, negctrlpasc_encoding.keys())

    select_codes = ['NEO{:03d}'.format(x) for x in range(59, 65)]
    select_group_name = 'Leukemia'
    negctrlpasc_list.append(select_group_name)
    group_dx_col(df, select_codes, select_group_name, negctrlpasc_encoding.keys())

    select_codes = ['NEO070']
    select_group_name = 'Secondary malignancies'
    negctrlpasc_list.append(select_group_name)
    group_dx_col(df, select_codes, select_group_name, negctrlpasc_encoding.keys())

    select_codes = ['NEO072']
    select_group_name = 'Neoplasms of unspecified nature or uncertain behavior'
    negctrlpasc_list.append(select_group_name)
    group_dx_col(df, select_codes, select_group_name, negctrlpasc_encoding.keys())

    select_codes = ['NEO073']
    select_group_name = 'Benign neoplasms'
    negctrlpasc_list.append(select_group_name)
    group_dx_col(df, select_codes, select_group_name, negctrlpasc_encoding.keys())
    ...
    for i, pasc in tqdm(enumerate(selected_screen_list, start=1), total=len(selected_screen_list)):
        ...
        out_file_balance = r'../data/recover/output/results/Paxlovid-{}-{}-{}-NEGCTRL-V3/{}-{}-results.csv'.format(
            args.cohorttype,
        ...
        df_summary.to_csv(
            '../data/recover/output/results/Paxlovid-{}-{}-{}-NEGCTRL-V3/{}-{}-evaluation_balance.csv'.format(
                args.cohorttype,
                _clean_name_(args.severity),
                'pcornet',  # '-select' if args.selectpasc else '',
                i,
        ...
        dfps.to_csv(
            '../data/recover/output/results/Paxlovid-{}-{}-{}-NEGCTRL-V3/{}-{}-evaluation_ps-iptw.csv'.format(
                args.cohorttype,
                _clean_name_(args.severity),
                'pcornet',  # '-select' if args.selectpasc else '',
                i,
                _clean_name_(pasc)))
        try:
            figout = r'../data/recover/output/results/Paxlovid-{}-{}-{}-NEGCTRL-V3/{}-{}-PS.png'.format(
                args.cohorttype,
                _clean_name_(args.severity),
                'pcornet',  # '-select' if args.selectpasc else '',
                i,
        ...
        km, km_w, cox, cox_w, cif, cif_w = weighted_KM_HR(
            ...
            fig_outfile=r'../data/recover/output/results/Paxlovid-{}-{}-{}-NEGCTRL-V3/{}-{}-km.png'.format(
                args.cohorttype,
                _clean_name_(args.severity),
                'pcornet',  # '-select' if args.selectpasc else '',
                i,
        ...
        try:
            ...
            if i % 2 == 0:
                pd.DataFrame(causal_results, columns=results_columns_name). \
                    to_csv(
                    r'../data/recover/output/results/Paxlovid-{}-{}-{}-NEGCTRL-V3/causal_effects_specific-snapshot-{}.csv'.format(
                        args.cohorttype,
                        _clean_name_(args.severity),
                        'pcornet',  # '-select' if args.selectpasc else '',
                        i))
            ...
            df_causal.to_csv(
                r'../data/recover/output/results/Paxlovid-{}-{}-{}-NEGCTRL-V3/causal_effects_specific-ERRORSAVE.csv'.format(
                    args.cohorttype,
                    _clean_name_(args.severity),
                    'pcornet',  # '-select' if args.selectpasc else '',
                ))
    ...
    df_causal.to_csv(
        r'../data/recover/output/results/Paxlovid-{}-{}-{}-NEGCTRL-V3/causal_effects_specific.csv'.format(
            args.cohorttype,
            _clean_name_(args.severity),
            'pcornet',  # '-select' if args.selectpasc else '',
        ))
