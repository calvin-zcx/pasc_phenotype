...
def parse_args():
    ...
    # More args
    args.data_file = r'../data/{}/output/character/matrix_cohorts_covid_4manuNegNoCovid_bool_{}-4negctrl.csv'.format(
        args.dataset,
        args.site)
...
if __name__ == "__main__":
    ...
    icd_pasc = utils.load(r'../data/mapping/icd_negative-outcome-control_mapping.pkl')
    ...
    # build grouped negative control pasc
    ...
    pasc_list.append(select_group_name)
    group_dx_col(df, select_codes, select_group_name, pasc_encoding.keys())
    ...
    for i, pasc in tqdm(enumerate(pasc_list, start=1), total=len(pasc_list)):
        ...
        model = ml.PropensityEstimator(
            ...
            'max_iter': 200,
            'random_state': 0}).cross_validation_fit(covs_array,
                                                                                                        covid_label,
                                                                                                        verbose=0)
        # , paras_grid = {
        #     'penalty': 'l2',
        ...
        out_file_balance = r'../data/{}/output/character/outcome/negctrl/DX-{}{}/{}-{}-results.csv'.format(
            args.dataset,
            args.severity,
        ...
        df_summary.to_csv(
            '../data/{}/output/character/outcome/negctrl/DX-{}{}/{}-{}-evaluation_balance.csv'.format(
                args.dataset,
                args.severity,
                '-select' if args.selectpasc else '',
                i, pasc))
        ...
        km, km_w, cox, cox_w, cif, cif_w = weighted_KM_HR(
            ...
            fig_outfile=r'../data/{}/output/character/outcome/negctrl/DX-{}{}/{}-{}-km.png'.format(
                args.dataset,
                args.severity,
                '-select' if args.selectpasc else '',
                i, pasc),
        ...
        try:
            ...
            df_causal.to_csv(
                r'../data/{}/output/character/outcome/negctrl/DX-{}{}/causal_effects_specific-ERRORSAVE.csv'.format(
                    args.dataset,
                    args.severity,
                    '-select' if args.selectpasc else '',))
    ...
    df_causal.to_csv(
        r'../data/{}/output/character/outcome/negctrl/DX-{}{}/causal_effects_specific.csv'.format(
            args.dataset,
            args.severity,
            '-select' if args.selectpasc else ''))
