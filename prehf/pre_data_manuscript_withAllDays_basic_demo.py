...
def parse_args():
    ...
    parser.add_argument('--cohorts', choices=['hf'],
                        default='hf', help='cohorts')
...
def _load_mapping():
    ...
    # 2023-11-5
    icd_ad = utils.load(r'../data/mapping/icd_AD_mapping.pkl')
    ad_encoding = utils.load(r'../data/mapping/AD_index_mapping.pkl')
    rxndc_addrug = utils.load(r'../data/mapping/rxndc_ADdrug_mapping.pkl')
    addrug_encoding = utils.load(r'../data/mapping/ADdrug_index_mapping.pkl')
    rxndc_hfdrug = utils.load(r'../data/mapping/rxndc_hfdrug_mapping.pkl')
    hfdrug_encoding = utils.load(r'../data/mapping/hfdrug_index_mapping.pkl')
    ...
    return icd_pasc, pasc_encoding, icd_cmr, cmr_encoding, icd_ccsr, ccsr_encoding, \
        ...
        icd_ad, ad_encoding, rxndc_addrug, addrug_encoding, rxndc_hfdrug, hfdrug_encoding
...
def _encoding_race(race):
    ...
    encoding = np.zeros((1, 7), dtype='int')
    if race == '01':
        encoding[0, 0] = 1  # 'American Indian or Alaska Native',
    elif race == '02':
        encoding[0, 1] = 1  # 'Asian',
    elif race == '04':
        encoding[0, 2] = 1  # 'Native Hawaiian or Other Pacific Islander,',
    elif race == '03':
        encoding[0, 3] = 1  # 'Black or African American',
    elif race == '05':
        encoding[0, 4] = 1  # 'White',
    elif race == '06':
        encoding[0, 5] = 1  # 'Multiple Race',
    else:
        encoding[0, 6] = 1  # 'Missing'
    return encoding
...
def _count_general_dx_withalldays(dx_list, icd_pasc, pasc_encoding, index_date):
    outcome_flag = np.zeros((1, len(pasc_encoding)), dtype='int')
    outcome_t2eall = [''] * len(pasc_encoding)
...
def _count_general_med_withalldays(med_list, rxndc_pasc, pasc_encoding, index_date):
    outcome_flag = np.zeros((1, len(pasc_encoding)), dtype='int')
    outcome_t2eall = [''] * len(pasc_encoding)
    ...
    for records in med_list:
        ...
        days = (med_date - index_date).days
        if rxnorm in rxndc_pasc:
            pasc_info = rxndc_pasc[rxnorm]
            pasc = pasc_info[0]
            rec = pasc_encoding[pasc]
            pos = rec[0]
...
def build_query_1and2_matrix(args):
    ...
    icd_pasc, pasc_encoding, icd_cmr, cmr_encoding, \
        ...
        icd_ad, ad_encoding, rxndc_addrug, addrug_encoding, rxndc_hfdrug, hfdrug_encoding = _load_mapping()

    # step 2: load cohorts pickle data
    print('In cohorts_characterization_build_data...')
    # if args.dataset == 'all':
    #     # sites = ['NYU', 'MONTE', 'COL', 'MSHS', 'WCM']
    #     sites = ['nyu', 'montefiore', 'columbia', 'mshs', 'wcm']
    # else:
    #     sites = [args.dataset, ]
    sites = [args.dataset, ]
    ...
    for site in tqdm(sites):
        print('Loading: ', site)
        input_file = r'../data/recover/output_hf/{}/cohorts_{}_{}.pkl'.format(site, args.cohorts, site)
        output_file_matrix = r'../data/recover/output_hf/{}/matrix_cohorts_{}_nout-withAllDays_{}-basic-demo.csv'.format(
            args.dataset, args.cohorts, args.dataset)

        # output_med_info = r'../data/recover/output_hf/{}/info_medication_cohorts_{}_{}.csv'.format(
        #     args.dataset, args.cohorts, args.dataset)
        #
        # output_dx_info = r'../data/recover/output_hf/{}/info_dx_cohorts_{}_{}.csv'.format(
        #     args.dataset, args.cohorts, args.dataset)
        ...
        for pid, item in tqdm(id_data.items(), total=len(id_data), mininterval=10):
            ...
            index_info, demo, dx_raw, med, covid_lab, encounter, procedure, obsgen, immun, death, vital = item
            flag, index_date, index_dx, index_dx_type, index_enc_type, index_age, index_enc_id = index_info
            if len(demo) == 0:
            ...
            # Other (American Indian or Alaska Native, Native Hawaiian or Other Pacific Islander, Multiple Race, Other)5
            # Missing (No Information, Refuse to Answer, Unknown, Missing)4
            race_array = np.zeros((n, 7), dtype='int16')
            ...
            # ad dx
            addx_array = np.zeros((n, 4), dtype='int16')  # from 37 --> 40, # added 2022-05-25
            addx_t2eall = []
            _column_names = ["mci", "ad", "dementia", "adrd_sd_ccw"]
            addx_column_names = ['ADDX:' + x for x in _column_names] + ['ADDX-tall:' + x for x in _column_names]

            admed_array = np.zeros((n, 5), dtype='int16')
            admed_t2eall = []
            _column_names = ['denepezil', 'memantine', 'tacrine', 'rivastigmine', 'galantamine']
            admed_column_names = ['ADMED:' + x for x in _column_names] + ['ADMED-tall:' + x for x in _column_names]

            hfmed_array = np.zeros((n, 21), dtype='int16')
            hfmed_t2eall = []
            _column_names = ['acebutolol', 'atenolol', 'betaxolol', 'bisoprolol', 'carteolol', 'carvedilol',
                             'labetalol',
            ...
            hfmed_column_names = ['HFMED:' + x for x in _column_names] + ['HFMED-tall:' + x for x in _column_names]

            # column_names = ['patid', 'site', 'hf', 'index date', 'hospitalized',
            ...
            #                addx_column_names + admed_column_names + hfmed_column_names
            column_names = ['patid', 'site', 'hf', 'index date', ] +  \
                           ['zip', 'dob', 'age', 'adi'] + \
                           gender_column_names + race_column_names + hispanic_column_names
            ...
            #
            # addx_array[i, :], addx_t2eall_1row = \
            #     _count_general_dx_withalldays(dx_raw, icd_ad, ad_encoding, index_date, )
            # addx_t2eall.append(addx_t2eall_1row)
            #
            # admed_array[i, :], admedt2eall_1row = \
            #     _count_general_med_withalldays(med, rxndc_addrug, addrug_encoding, index_date, )
            # admed_t2eall.append(admedt2eall_1row)
            #
            # hfmed_array[i, :], hfmedt2eall_1row = \
            #     _count_general_med_withalldays(med, rxndc_hfdrug, hfdrug_encoding, index_date, )
            # hfmed_t2eall.append(hfmedt2eall_1row)
            ...
            data_array = np.hstack((np.asarray(pid_list).reshape(-1, 1),
                                    ...
                                    # smoking_array,
                                    # addx_array,
                                    # np.asarray(addx_t2eall),
                                    # admed_array,
                                    # np.asarray(admed_t2eall),
                                    # hfmed_array,
                                    # np.asarray(hfmed_t2eall),
                                    ))

            df_data = pd.DataFrame(data_array, columns=column_names)

            df_data.to_csv(output_file_matrix, mode=mode, header=header, index=False)
            if header:
                header = False
                mode = "a"

        print('Done! Dump data matrix to {}'.format(output_file_matrix))
