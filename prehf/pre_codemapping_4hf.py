...
def ICD_to_AD():
    ...
    start_time = time.time()
    ad_list_file = r'../data/mapping/adrd_dx.xlsx'
    df_ad = pd.read_excel(ad_list_file, sheet_name=None, dtype=str)
    # 'mci', 'ad', 'dementia', 'dementia drug', 'adrd_sd_ccw'
    sheet_name = ['mci', 'ad', 'dementia', 'adrd_sd_ccw']
    dfs = []
    ...
    df_ad_list = pd.concat(dfs)
    print('df_ad_list.shape', df_ad_list.shape)

    df_ad_list['dx code'] = df_ad_list['dx code'].apply(lambda x: x.strip().upper().replace('.', ''))

    ad_codes = df_ad_list['dx code']
    ...
    for index, row in df_dim.iterrows():
        category = row[0]
        cnt = row[1]
        ad_index[category] = [index, cnt]

    print('len(icd_ad):', len(icd_ad))
    output_file = r'../data/mapping/icd_AD_mapping.pkl'
    utils.check_and_mkdir(output_file)
    pickle.dump(icd_ad, open(output_file, 'wb'))
    print('dump done to {}'.format(output_file))

    print('len(ad_index):', len(ad_index))
    output_file = r'../data/mapping/AD_index_mapping.pkl'
    utils.check_and_mkdir(output_file)
    pickle.dump(ad_index, open(output_file, 'wb'))
    print('dump done to {}'.format(output_file))

    print('Time used:', time.strftime("%H:%M:%S", time.gmtime(time.time() - start_time)))
    return icd_ad, ad_index, df_ad_list
...
def rxndc_to_ADdrug():
    ...
    start_time = time.time()
    ad_list_file = r'../data/mapping/adrd_dx.xlsx'
    df_addrug = pd.read_excel(ad_list_file, sheet_name='dementia drug', dtype=str)

    print('df_addrug.shape', df_addrug.shape)
    df_addrug['code'] = df_addrug['code'].apply(lambda x: x.strip())

    ad_codes = df_addrug['code']
    ad_codes_set = set(ad_codes)
    print('Load compiled AD drug list done from {}\nlen(ad_codes)'.format(ad_list_file),
          len(ad_codes), 'len(ad_codes_set):', len(ad_codes_set))

    rxndc_addrug = {}
    addrug_index = {}

    for index, row in df_addrug.iterrows():
        rxndc = row['code']

        drug_name = row['name']
        drug_category = row['drug category']
        codetype = row['code type']
        rxndc_addrug[rxndc] = [drug_category, codetype, drug_name]

    df_dim = df_addrug['drug category'].value_counts(sort=False).reset_index()
    for index, row in df_dim.iterrows():
        category = row[0]
        cnt = row[1]
        addrug_index[category] = [index, cnt]

    print('len(rxndc_addrug):', len(rxndc_addrug))
    output_file = r'../data/mapping/rxndc_ADdrug_mapping.pkl'
    utils.check_and_mkdir(output_file)
    pickle.dump(rxndc_addrug, open(output_file, 'wb'))
    print('dump done to {}'.format(output_file))

    print('len(addrug_index):', len(addrug_index))
    output_file = r'../data/mapping/ADdrug_index_mapping.pkl'
    utils.check_and_mkdir(output_file)
    pickle.dump(addrug_index, open(output_file, 'wb'))
    print('dump done to {}'.format(output_file))
