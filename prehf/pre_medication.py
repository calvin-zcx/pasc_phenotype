...
def parse_args():
    ...
    args.med_admin_output = r'../data/recover/output_hf/{}/_med_admin_{}.pkl'.format(args.dataset, args.dataset)
    args.prescribe_output = r'../data/recover/output_hf/{}/_prescribing_{}.pkl'.format(args.dataset, args.dataset)
    args.dispensing_output = r'../data/recover/output_hf/{}/_dispensing_{}.pkl'.format(args.dataset, args.dataset)

    args.output_file = r'../data/recover/output_hf/{}/medication_{}.pkl'.format(args.dataset, args.dataset)
            ...
            if pd.isna(rxnorm) or pd.isna(start_date):
                ...
                if not selected_patients:
                    ...
                    if patid in selected_patients:
                        id_med[patid].append(
                            (start_date, rxnorm, days, encid,
                             'pr'))  # 2022-10-24 add encounter id and prescribeing table id
                        n_recorded_row += 1
                    else:
                        n_not_in_list_row += 1
        ...
        for index, row in chunk.iterrows():
            ...
            if pd.isna(rxnorm) or pd.isna(start_date):
                ...
                if not selected_patients:
                    ...
                    if patid in selected_patients:
                        id_med[patid].append((start_date, rxnorm, days, encid, 'ma'))
                        n_recorded_row += 1
                    else:
                        n_not_in_list_row += 1
...
def read_dispensing(input_file, output_file='', selected_patients={}):
    """
    :param selected_patients:
    :param output_file:
    ...
    """
    start_time = time.time()
    print('In read_dispensing, input_file:', input_file, 'output_file:', output_file)
    if selected_patients:
        print('using selected_patients, len(selected_patients):', len(selected_patients))
    ...
    for chunk in tqdm(pd.read_sql(sql_query, connection, chunksize=chunksize), total=n_chunk, mininterval=5):
        ...
        for index, row in chunk.iterrows():
            patid = row['PATID']
            rx_start_date = clean_date_str(row['DISPENSE_DATE'])
            rxnorm = row['NDC']
            ...
            if pd.isna(rxnorm) or pd.isna(start_date):
                ...
                if not selected_patients:
                    # replace enc_id to dispense_source, no enc_id in dispense table
                    id_med[patid].append(
                        (start_date, rxnorm, days, dispense_source, 'ds'))
                    n_recorded_row += 1
                else:
                    if patid in selected_patients:
                        id_med[patid].append((start_date, rxnorm, days, dispense_source, 'ds'))
                        n_recorded_row += 1
                    else:
                        n_not_in_list_row += 1
    ...
    try:
        dfs = pd.concat(dfs)
        print('dfs.shape', dfs.shape)
        print('Time range of dispensing table  of selected patients [DISPENSE_DATE]:',
...
if __name__ == '__main__':
    ...
    print('DISPENSE_DATE done, len(id_med2):', len(id_med3))

    print("step 4. combine both")
    id_med = combine_2_id_med(id_med1, id_med2,)
    id_med = combine_2_id_med(id_med, id_med3, args.output_file)
