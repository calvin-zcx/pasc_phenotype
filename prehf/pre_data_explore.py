...
def parse_args():
    ...
    # args.output_file_query12 = r'../data/recover/output_hf/{}/matrix_cohorts_{}_cnt_{}.csv'.format(
    #     args.dataset,
    #     args.cohorts,
    #     args.dataset)

    # args.output_file_query12_bool = r'../data/recover/output_hf/{}/matrix_cohorts_{}_boolbase-nout-withAllDays_{}.csv'.format(
    #     args.dataset,
    #     args.cohorts,
    #     args.dataset)
...
def _load_mapping():
    ...
    ad_index = utils.load(r'../data/mapping/AD_index_mapping.pkl')
    rxndc_addrug = utils.load(r'../data/mapping/rxndc_ADdrug_mapping.pkl')
    addrug_index = utils.load(r'../data/mapping/ADdrug_index_mapping.pkl')
    ...
    return icd_pasc, pasc_encoding, icd_cmr, cmr_encoding, icd_ccsr, ccsr_encoding, \
           ...
           icd_ad, ad_index, rxndc_addrug, addrug_index
...
def build_query_1and2_matrix(args):
    ...
    icd_ad, ad_index, rxndc_addrug, addrug_index,   = _load_mapping()
    ...
    for site in tqdm(sites):
        ...
        output_file_query12_bool = r'../data/recover/output_hf/{}/matrix_cohorts_{}_{}.csv'.format(
        ...
        for pid, item in tqdm(id_data.items(), total=len(id_data), mininterval=10):
            ...
            # [row, demo, dx, med, hfdx, enc, procedure, obsgen, immun, death, vital]
            index_info, demo, dx_raw, med, covid_lab, encounter, procedure, obsgen, immun, death, vital = item
            # flag   (dx_date, dx, dx_type, enc_type, age, enc_id)
            flag, index_date, index_dx, index_dx_type, index_age, index_enc_id = index_info
            if len(demo) == 0:
            ...
            # mci, ad, dementia, adrd_sd_ccw
            outcome_flag = np.zeros((n, 4), dtype='int16')
            outcome_t2e = np.zeros((n, 4), dtype='int16')
            outcome_baseline = np.zeros((n, 4), dtype='int16')
            outcome_t2eall = []
            ...
            # dementia drug
            outcome_med_flag = np.zeros((n, 1), dtype='int16')
            outcome_med_t2e = np.zeros((n, 1), dtype='int16')
            outcome_med_baseline = np.zeros((n, 1), dtype='int16')
            ...
            column_names = ['patid', 'site', 'covid', 'index date', 'hospitalized',
                           ...
                           ['zip', 'dob', 'age', 'adi'] + utilization_count_names + ['bmi'] + \
                           age_column_names + \
                           gender_column_names + race_column_names + hispanic_column_names + \
                           social_column_names + utilization_column_names + \
                           bmi_names + smoking_names + \
                           dx_column_names + med_column_names + \
                           outcome_column_names + outcome_med_column_names
