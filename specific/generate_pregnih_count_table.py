...
if __name__ == "__main__":
    ...
    print('In cohorts_characterization_build_data...')
    if args.site == 'all':
        sites = ['wcm_pcornet_all', 'nyu_pcornet_all', 'montefiore_pcornet_all', 'mshs_pcornet_all',
                 'columbia_pcornet_all']
        print('len(sites), sites:', len(sites), sites)
    else:
        sites = [args.site, ]
    ...
    for ith, site in tqdm(enumerate(sites)):
        ...
        data_file = r'../data/recover/output/{}/matrix_preg_roughcal_{}-nbaseout-alldays-preg_{}-V2.csv'.format(
            site, 'covid_posnegpreg', site)
    ...
    results_df = detailed_race_eth_table(df)
    results_df.to_csv('pregnant_race_the_sex.csv')

    zz
    out_file = r'Table-pregrough-2025-1-23.xlsx'
    output_columns = ['All', ]
    row_names = []
    records = []
    ...
    row_names.extend(pregtreat_names)
    records.extend(
        [[_percentage_str(df[c]), ]
         for c in col_names])
