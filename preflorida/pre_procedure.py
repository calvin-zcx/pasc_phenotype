...
def parse_args():
    ...
    args.input_file = r'../data/oneflorida/{}/PROCEDURES.csv'.format(args.dataset)
    args.patient_list_file = r'../data/oneflorida/output/{}/patient_covid_lab_{}.pkl'.format(args.dataset, args.dataset)
    args.output_file = r'../data/oneflorida/output/{}/procedures_{}.pkl'.format(args.dataset, args.dataset)

    args.input_file2 = r'../data/oneflorida/{}/OBS_GEN.csv'.format(args.dataset)
    args.output_file2 = r'../data/oneflorida/output/{}/obs_gen_{}.pkl'.format(args.dataset, args.dataset)
...
def read_procedure(input_file, output_file='', selected_patients={}):
    ...
    for chunk in sasds:  # , meta
        ...
        for index, row in chunk.iterrows():
            ...
            enc_type = np.nan  # row['ENC_TYPE'] # not in oneflorida data
            px = row['PX']
            px_type = row["PX_TYPE"]
            px_date = np.nan  # row["PX_DATE"]  # not in oneflorida data
