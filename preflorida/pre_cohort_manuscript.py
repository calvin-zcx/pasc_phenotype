...
def parse_args():
    ...
    args.covid_lab_file = r'../data/oneflorida/output/{}/patient_covid_lab_{}.pkl'.format(args.dataset, args.dataset)
    args.demo_file = r'../data/oneflorida/output/{}/patient_demo_{}.pkl'.format(args.dataset, args.dataset)
    args.dx_file = r'../data/oneflorida/output/{}/diagnosis_{}.pkl'.format(args.dataset, args.dataset)
    args.med_file = r'../data/oneflorida/output/{}/medication_{}.pkl'.format(args.dataset, args.dataset)
    args.enc_file = r'../data/oneflorida/output/{}/encounter_{}.pkl'.format(args.dataset, args.dataset)
    # added 2022-02-02
    args.pro_file = r'../data/oneflorida/output/{}/procedures_{}.pkl'.format(args.dataset, args.dataset)
    args.obsgen_file = r'../data/oneflorida/output/{}/obs_gen_{}.pkl'.format(args.dataset, args.dataset)
...
def read_preprocessed_data(args):
    ...
    id_lab = utils.load(args.covid_lab_file, chunk=4)
    id_demo = utils.load(args.demo_file, chunk=4)
    id_dx = utils.load(args.dx_file, chunk=4)
    id_med = utils.load(args.med_file, chunk=4)
    id_enc = utils.load(args.enc_file, chunk=4)
    id_pro = utils.load(args.pro_file, chunk=4)
...
def integrate_data_and_apply_eligibility(args):
    ...
    def _covid_label_mapping(y):
        y = y.upper()
        if y in ('POSITIVE', 'DETECTED', 'PRESUMPTIVE POSITIVE'):
            return 'POSITIVE'
        elif y in ('NOT DETECTED', 'NEGATIVE'):
            return 'NEGATIVE'
        else:
            return y  # 'NI'

    for i, (pid, row) in enumerate(id_lab.items()):
        v_lables = [_covid_label_mapping(x[2]) for x in row]
...
if __name__ == '__main__':
    # python pre_cohort_manuscript.py --dataset all 2>&1 | tee  log/pre_cohort_manuscript_all_oneflorida.txt
