...
def read_covid_lab_and_generate_label(args, output_file='', id_demo={}):
    ...
    print('df1 Unique patid:', len(df1['PATID'].unique()))
    ...
    print('df2 Unique patid:', len(df2['PATID'].unique()))

    df = pd.concat([df1, df2])
    print('df.shape', df.shape)
    print('df.columns', df.columns)
    print('combined df Unique patid:', len(df['PATID'].unique()))

    print('Time range of All Covid Test:', df["RESULT_DATE"].describe(datetime_is_numeric=True))
    print('PCR/Antigen/Antibody LAB_LOINC Count:', df["LAB_LOINC"].value_counts(dropna=False))
    print('PCR/Antigen/Antibody RESULT_QUAL Count:', df["RESULT_QUAL"].value_counts(dropna=False))
    ...
    print('PCR/Antigen LAB_LOINC Count:', df_covid["LAB_LOINC"].value_counts(dropna=False))
    print('PCR/Antigen RESULT_QUAL Count:', df_covid["RESULT_QUAL"].value_counts(dropna=False))
    ...
    for index, row in tqdm(df_covid.iterrows(), total=len(df_covid)):
        ...
        # newly add pd.isna(result_label) 2022-03-10
        if pd.isna(lab_code) or pd.isna(lab_date) or pd.isna(result_label):
            n_discard_row += 1
        else:
            if patid in id_demo:
