    ...
    for records in dx_list:
        ...
        if ecs._is_in_followup(dx_date, index_date):
            ...
            if flag:  # if icd in icd_pasc:
                ...
                if enc_type == 'EI' or enc_type == 'IP' or enc_type == 'OS':
                    outcome_enctype[pos, 0] += 1
                elif enc_type == 'AV' or enc_type == 'OA' or enc_type == 'TH':
                    outcome_enctype[pos, 1] += 1
                elif enc_type == 'ED':
                    outcome_enctype[pos, 2] += 1
                else:
...
def build_query_1and2_matrix(args):
    ...
    for site in tqdm(sites):
        ...
        df_pasc_enc = pd.DataFrame(outcome_enctype, index=list(pasc_encoding.keys()),
                                   columns=['inpatient', 'outpatient', 'emergency', 'other'])
