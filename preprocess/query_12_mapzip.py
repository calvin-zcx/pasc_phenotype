...
def parse_args():
    ...
    args.output_file_query12 = r'../data/V15_COVID19/output/character/zip/matrix_cohorts_{}_query12_encoding_cnt_{}.csv'.format(
        args.cohorts,
        args.dataset)
    args.output_file_query12_bool = r'../data/V15_COVID19/output/character/zip/matrix_cohorts_{}_query12_encoding_bool_{}.csv'.format(
        args.cohorts,
        args.dataset)
...
def build_query_1and2_matrix(args):
    ...
    for site in tqdm(sites):
        ...
        column_names = ['patid', 'site', 'covid', 'hospitalized', 'ventilation', ] + \
                       ["zipcode", "state", "city", "nation_adi", "state_adi"] + age_column_names + \
        ...
        for i, (pid, item) in tqdm(enumerate(id_data.items()), total=len(id_data)):
            ...
            zip_list.append([zipcode, state, city, nation_adi, state_adi])
        ...
        data_array = np.hstack((np.asarray(pid_list).reshape(-1, 1),
                                ...
                                np.asarray(covid_list).reshape(-1, 1),
                                np.asarray(hospitalized_list).reshape(-1, 1),
                                np.asarray(ventilation_list).reshape(-1, 1),
                                np.asarray(zip_list),
                                age_array,
                                gender_array,
                                race_array,
...
if __name__ == '__main__':
    # python query_12_mapzip.py --dataset ALL --cohorts pasc_incidence 2>&1 | tee  log/query_12_mapzip_ALL_pasc_incidence.txt
    # python query_12_mapzip.py --dataset ALL --cohorts pasc_prevalence 2>&1 | tee  log/query_12_mapzip_ALL_pasc_prevalence.txt
    # python query_12_mapzip.py --dataset ALL --cohorts covid 2>&1 | tee  log/query_12_mapzip_ALL_covid.txt
