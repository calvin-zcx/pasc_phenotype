...
def _encoding_vaccine(pro_list, immun_list, vaccine_column_names, vaccine_codes, index_date):
    ...
    #                         'J&J fully vaccinated - Post-index',
    #                         'Not fully vaccinated - Pre-index',
    #                         'Not fully vaccinated - Post-index',
    #                         'mRNA fully vaccinated - anytime',
    #                         'J&J fully vaccinated - anytime',
...
def _encoding_vaccineV2(pro_list, immun_list, vaccine_column_names, vaccine_codes, index_date):
    ...
    encoding[0, 0] = int(
        (_fully_vaccined_mrna(mrna_pre) or _fully_vaccined_jj(jj_pre)))  # 'Fully vaccinated - Pre-index',
    encoding[0, 1] = int(
        (_fully_vaccined_mrna(mrna_post) or _fully_vaccined_jj(jj_post)))  # 'Fully vaccinated - Post-index',
    encoding[0, 2] = int(
        (not _fully_vaccined_mrna(mrna_pre)) and (len(mrna_pre) > 0))  # 'Partially vaccinated - Pre-index'
    encoding[0, 3] = int(
        (not _fully_vaccined_mrna(mrna_post)) and (len(mrna_post) > 0))  # 'Partially vaccinated - Post-index',
...
def _encoding_vaccineV3(pro_list, immun_list, vaccine_column_names, vaccine_codes, index_date, source='all'):
    # vaccineV2_column_names = ['Fully vaccinated - Pre-index',
    #                           'Fully vaccinated - Post-index',
    #                           'Partially vaccinated - Pre-index',
    #                           'Partially vaccinated - Post-index',
    ...
    #     'vax_unspec',
    #     'pfizer_any', 'moderna_any', 'janssen_any', 'any_mrna']
    assert source in ['all', 'procedure', 'immunization']

    mrna = []
    jj = []
    encoding = np.zeros((1, len(vaccine_column_names)), dtype='int')
    if source in ['all', 'procedure']:
        for records in pro_list:
            px_date, px, px_type, enc_type, enc_id = records
            if px in vaccine_codes['any_mrna']:
                mrna.append((px_date, px))
...
def _encoding_vaccine_aux(pro_list, immun_list, _vaccine_column_names, vaccine_codes, index_date):
    encoding = np.zeros((1, len(_vaccine_column_names)), dtype='int')
    for records in pro_list:
        px_date, px, px_type, enc_type, enc_id = records
...
def build_query_1and2_matrix(args):
    ...
    for site in tqdm(sites):
        ...
        vaccineV3_pro_column_names = ['Fully vaccinated - Pre-index in Procedure',
                                      'Fully vaccinated - Post-index in Procedure',
                                      'Partially vaccinated - Pre-index in Procedure',
                                      'Partially vaccinated - Post-index in Procedure',
                                      'No evidence - Pre-index in Procedure',
                                      'No evidence - Post-index in Procedure',
                                      ]
        vaccineV3_pro_array = np.zeros((n, 6), dtype='int16')

        vaccineV3_immu_column_names = ['Fully vaccinated - Pre-index in Immunization',
                                       'Fully vaccinated - Post-index in Immunization',
                                       'Partially vaccinated - Pre-index in Immunization',
                                       'Partially vaccinated - Post-index in Immunization',
                                       'No evidence - Pre-index in Immunization',
                                       'No evidence - Post-index in Immunization',
                                       ]
        vaccineV3_immu_array = np.zeros((n, 6), dtype='int16')
        ...
        # vaccine_preindex_column_names = ['pre_' + x for x in vaccine_aux_column_names]
        # vaccine_postindex_column_names = ['post_' + x for x in vaccine_aux_column_names]
        vaccine_aux_array = np.zeros((n, 20), dtype='int16')
        ...
        for pid, item in tqdm(id_data.items(), total=len(
            ...
            vaccineV2_array[i, :] = _encoding_vaccineV2(procedure, immun, vaccineV2_column_names, vaccine_codes,
                                                        index_date)

            vaccineV3_pro_array[i, :] = _encoding_vaccineV3(procedure, immun, vaccineV2_column_names, vaccine_codes,
                                                            index_date, source='procedure')

            vaccineV3_immu_array[i, :] = _encoding_vaccineV3(procedure, immun, vaccineV2_column_names, vaccine_codes,
                                                             index_date, source='immunization')
...
def pasc_specific_cohorts_characterization_analyse(cohorts, dataset='ALL', severity='',
    ...
    df_data.loc[:, _vaccine_column_names] = (df_data.loc[:, _vaccine_column_names].astype('int') >= 1).astype('int')
