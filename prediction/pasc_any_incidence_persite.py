...
def load_file_per_site(sites=['wcm', 'nyu', 'mshs', 'montefiore', 'columbia']):
    start_time = time.time()
    print('In load_file_per_site, sites:', sites)
    vdf = []
    vdfec = []
    for site in sites:
        data_file = r'../data/recover/output/{}/matrix_cohorts_covid_4manuNegNoCovidV2_boolbase-nout-withAllDays_{}.csv'.format(
            site,
            site)
        data_ec = r'../data/recover/output/{}/cohorts_covid_4manuNegNoCovidV2_{}_info.csv'.format(site, site)
        print('Load file:', data_file)
        _df_ec = pd.read_csv(data_ec)
        _df = pd.read_csv(data_file, dtype={'patid': str, 'site': str, 'zip': str}, parse_dates=['index date'])
        _selected_cols = [x for x in _df.columns if
                          (
                              x.startswith('dx-out@')
                              # or x.startswith('med-out@')
                              # or x.startswith('covidmed-out@')
                          )]
        _df.loc[:, _selected_cols] = (_df.loc[:, _selected_cols].astype('int') >= 1).astype('int')

        print(site, _df.shape)
        vdf.append(_df)
        vdfec.append(_df_ec)
        print(site, ' done! time used:', time.strftime("%H:%M:%S", time.gmtime(time.time() - start_time)))

    dfec_sum = reduce(lambda x, y: x.add(y, fill_value=0), vdfec)
    df = pd.concat(vdf, axis=0, ignore_index=True)
    print('load_file_per_site Done! Total Time used:',
          time.strftime("%H:%M:%S", time.gmtime(time.time() - start_time)))
    return df, dfec_sum, vdfec


def build_incident_pasc_from_all_positive_withAllTimeRecords(sites, broad=True, nthreshold=1, dump=False):
    start_time = time.time()
    print('In build_data_from_all_positive')
    print('Step1: Load Covid positive data  file:', sites)

    df, dfec_sum, vdfec = load_file_per_site(sites)
    ...
    df['pasc-flag-2samedx30days'] = 0
    df['pasc-t2e-2samedx30days'] = np.nan
    ...
    for index, rows in tqdm(df.iterrows(), total=df.shape[0]):
        ...
        for tlist in time_all_list:
            tlist = np.array(stringlist_2_list(tlist))
            tmin = tlist.min()
            tmax = tlist.max()
            if tmax - tmin >= nthreshold:
                df.loc[index, 'pasc-flag-2samedx30days'] = 1
                if pd.isna(df.loc[index, 'pasc-t2e-2samedx30days']) or (tmin < df.loc[index, 'pasc-t2e-2samedx30days']):
                    df.loc[index, 'pasc-t2e-2samedx30days'] = tmin
    ...
    return df, df_pasc_info, dfec_sum, vdfec


def build_incident_pasc_from_all_positive_withinOrgan(sites, broad=True, nthreshold=30, dump=False):
    start_time = time.time()
    print('In build_data_from_all_positive')
    print('Step1: Load Covid positive data  file:', sites)
...
def build_incident_pasc_from_all_positive_withinOrgan_old(data_file, broad=True, nthreshold=30, dump=False):
    start_time = time.time()
    print('In build_data_from_all_positive')
    print('Step1: Load Covid positive data  file:', data_file)
...
def print_incidence_table(cneg, cpos, flag_col, per=100, neg_denom=0, pos_denom=0):
    neg_total = len(cneg) + neg_denom
    neg_yes = len(cneg.loc[cneg[flag_col] > 0, :])
    neg_no = len(cneg.loc[cneg[flag_col] == 0, :]) + neg_denom

    pos_total = len(cpos) + pos_denom
    pos_yes = len(cpos.loc[cpos[flag_col] > 0, :])
    pos_no = len(cpos.loc[cpos[flag_col] == 0, :]) + pos_denom
...
if __name__ == '__main__':
    ...
    neg_denom = int(dfec_sum.loc[dfec_sum['ec'].str.startswith(r'_eligibility_followup_any_dx'), 'exclude N Neg'].values[0])
    pos_denom = int(dfec_sum.loc[dfec_sum['ec'].str.startswith(r'_eligibility_followup_any_dx'), 'exclude N Pos'].values[0])
    # Incidence table
    print('Incidence Table')
    print_incidence_table(df.loc[(df['covid'] == 0), :],
                          df.loc[(df['covid'] == 1), :], 'pasc-flag', per=100)
    print('Incidence Table, no any dx in followup contraint')
    print_incidence_table(df.loc[(df['covid'] == 0), :],
                          df.loc[(df['covid'] == 1), :], 'pasc-flag', per=100, neg_denom=neg_denom, pos_denom=pos_denom)

    # not for within organ
    if 'pasc-flag-2samedx30days' in df.columns:
        print('Incidence 2samedx-30 days apart Table')
        print_incidence_table(df.loc[(df['covid'] == 0), :],
                              df.loc[(df['covid'] == 1), :], 'pasc-flag-2samedx30days', per=100)
        print('Incidence 2samedx-30 days apart Table, no any dx in followup contraint')
        print_incidence_table(df.loc[(df['covid'] == 0), :],
                              df.loc[(df['covid'] == 1), :], 'pasc-flag-2samedx30days', per=100,
                              neg_denom=neg_denom, pos_denom=pos_denom)

    print('Incidence 2dx-30 days apart Table')
    print_incidence_table(df.loc[(df['covid'] == 0), :],
                          df.loc[(df['covid'] == 1), :], 'pasc-flag-2dx30days', per=100)
    print('Incidence 2dx-30 days apart Table, no any dx in followup contraint')
    print_incidence_table(df.loc[(df['covid'] == 0), :],
                          df.loc[(df['covid'] == 1), :], 'pasc-flag-2dx30days', per=100,
                          neg_denom=neg_denom, pos_denom=pos_denom)
