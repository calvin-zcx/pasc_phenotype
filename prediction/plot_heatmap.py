...
def combine_risk_p_value_with_interaction_revision2(database='INSIGHT', severity='all'):
    # df_pasc_info = pd.read_excel(
    #     r'C:/Users/zangc/Documents/Boston/workshop/2021-PASC/prediction/PASC_risk_factors_predictability.xlsx',
    #     sheet_name='person_counts_LR_res')
    df_pasc_info = pd.read_excel(
        ...
        for pasc, pasc_raw in zip(pasc_name_simple, pasc_name_raw):
            if pasc == 'Any PASC':
                fname = dir_path + 'any_pasc/' + 'any-at-least-1-pasc' + \
                        '-riskFactor-' + database + '-positive-{}.csv'.format(severity)
                fname_inter = dir_path + 'any_pasc/' + 'any-at-least-1-pasc' + \
                              '-riskFactor-' + database + '-all-{}-interaction.csv'.format(severity)
            else:
                fname = dir_path + 'every_pasc/' + 'PASC-' + pasc_raw.replace('/', '_') + \
                        '-riskFactor-' + database + '-positive-{}.csv'.format(severity)
            ...
            # add organ info
            df_result['Organ Domain'] = organ
            if df_row is not None:
                df_row = df_row.append(df_result)
            else:
                df_row = df_result

    df_row.to_csv(dir_path + 'combined_row_format_with_interaction-{}_revision2.csv'.format(severity))
...
def build_heat_map_from_selected_rows_plot_filteringByinteraction_revision2(
        database='INSIGHT',
        p_val_threshold=0.05 / 89,
        selected_cols=False,
        #interactionge1=False,
    ...
    df_row = pd.read_csv(dir_path + 'combined_row_format_with_interaction-{}_revision2.csv'.format(severity))
    print('df_row.shape:', df_row.shape)
    # if not interactionge1:
    #     df = df_row.loc[(df_row['p-Value'] < p_val_threshold) & (df_row['HR'] > 1), :].copy()
    # else:
    ...
    pasc_new = []
    # for key in pasc_name_list:
    #     if (key in pascs) and (key not in drop_pascs):
    #         pasc_new.append(key)
    # assert len(pasc_new) == len(pascs)
    print('to exclude pascs:', drop_pascs)
    for key in pascs:
        if key not in drop_pascs:
            pasc_new.append(key)
    pascs = pasc_new
    ...
    for key, row in df.iterrows():
        ...
        # if p_int > 0.05:
        #     continue
        if pasc not in pasc_id:
            print(pasc, 'not in pasc_id')
            continue
        ...
        if hr_int <= 1:
            highlight_cell1.append((j, i))

        if hr_int > 1 and p_int > 0.05:
            if (pasc, cov) not in drop_highlight:
                highlight_cell2.append((j, i))
            else:
                print('not highlight cell:', (pasc, cov))

    df_data = pd.DataFrame(data, index=pascs, columns=covs)  # [re.sub("\(.*?\)", "", x.replace('DX: ', '')) for x in covs])
    # if drop_cols:
    #     print('drop columns: ', drop_cols)
    #     print('before drop, df_data.shape:', df_data.shape)
    #     df_data = df_data.drop(columns=drop_cols)
    ...
    for cell in highlight_cell1:
        ax.add_patch(Rectangle(cell, 1, 1, fill=False, edgecolor='crimson', lw=2, linestyle='--', hatch='\\'))

    for cell in highlight_cell2:
        ax.add_patch(Rectangle(cell, 1, 1, fill=False, edgecolor='green', lw=2, linestyle='--', hatch='\\'))
    ...
    plt.savefig(dir_path + 'figure/R2-sensitivityHighlight-risk_heat_map_{}-p{:.6f}-{}-{}{}{}-pint005.png'.format(
        severity,
        p_val_threshold,
        '', #'-interHRGe1' if interactionge1 else '-interHRNotUsed',
        '', #'-interP{}'.format(interact_p_val_threhold) if interact_p_val_threhold is not None else '-interPNotUsed',
    ...
    plt.savefig(dir_path + 'figure/R2-sensitivityHighlight-risk_heat_map_{}-p{:.6f}-{}-{}{}{}-pint005.pdf'.format(
        severity,
        p_val_threshold,
        '', #'-interHRGe1' if interactionge1 else '-interHRNotUsed',
        '', #'-interP{}'.format(interact_p_val_threhold) if interact_p_val_threhold is not None else '-interPNotUsed',
...
def build_heat_map_from_selected_rows_plot_revision2(
        database='INSIGHT',
        p_val_threshold=0.05 / 89,
        selected_cols=False,
        interactionge1=False,
    ...
    if highlightinterle1:
        for cell in highlight_cell:
            ax.add_patch(Rectangle(cell, 1, 1, fill=False, edgecolor='darkmagenta', lw=2))  # crimson  Purple
    ...
    plt.savefig(dir_path + 'figure/R2-risk_heat_map_{}-p{:.6f}-{}-{}{}{}-pint005.png'.format(
        severity,
        p_val_threshold,
        '-interHRGe1' if interactionge1 else '-interHRNotUsed',
        '-interP{}'.format(interact_p_val_threhold) if interact_p_val_threhold is not None else '-interPNotUsed',
    ...
    plt.savefig(dir_path + 'figure/R2-risk_heat_map_{}-p{:.6f}-{}-{}{}{}-pint005.pdf'.format(
        severity,
        p_val_threshold,
        '-interHRGe1' if interactionge1 else '-interHRNotUsed',
        '-interP{}'.format(interact_p_val_threhold) if interact_p_val_threhold is not None else '-interPNotUsed',
...
def build_heat_map_from_inpatient_vs_outpatient_revision2(
        database='INSIGHT',
        p_val_threshold=0.05 / 89,
        selected_cols=False,
        interactionge1=False,
    ...
    df_row1 = pd.read_csv(dir_path + 'combined_row_format_with_interaction-outpatient_revision2.csv')
    df_row2 = pd.read_csv(dir_path + 'combined_row_format_with_interaction-inpatienticu_revision2.csv')
    pascs_superset = list(df_row1['pasc'].unique())
    ...
    pasc_new = []
    print('to exclude pascs:', drop_pascs)
    print('len(pascs_superset)', len(pascs_superset))
    print('len(set(df1[pasc].unique()))', len(set(df1['pasc'].unique())))
    print('len(set(df2[pasc].unique()))', len(set(df2['pasc'].unique())))
    print('len(pascs)', len(pascs))

    for key in pascs_superset:
        if (key in pascs) and (key not in drop_pascs):
            pasc_new.append(key)
    pascs = pasc_new

    print('Final len(pascs)', len(pascs))
    ...
    if drop_cols:
        ...
        print('before drop, len(covs1):', len(covs1))
        covs_new = [x for x in covs1 if x not in drop_cols]
        covs1 = covs_new
        print('after drop, len(covs1):', len(covs1))

        print('before drop, len(covs2):', len(covs2))
        covs_new = [x for x in covs2 if x not in drop_cols]
        covs2 = covs_new
        print('after drop, len(covs2):', len(covs2))
    ...
    # axes[1].set_title('Not Hospitalized', fontdict={'fontsize': 15, 'fontweight': 'medium'})
    if highlightinterle1:
        for cell in highlight1_cell:
            axes[0].add_patch(Rectangle(cell, 1, 1, fill=False, edgecolor='darkmagenta', lw=2))  # crimson  Purple

        for cell in highlight2_cell:
            axes[1].add_patch(Rectangle(cell, 1, 1, fill=False, edgecolor='darkmagenta', lw=2))
    ...
    utils.check_and_mkdir(dir_path + 'figure/')
    plt.savefig(dir_path + 'figure/R2_risk_heat_map_{}-p{:.6f}-{}-{}{}.png'.format(
        'inpatientVSoutpatient',
        p_val_threshold,
        '-interHRGe1' if interactionge1 else '-interHRNotUsed',
        '-interP{}'.format(interact_p_val_threhold) if interact_p_val_threhold is not None else '-interPNotUsed',
    ...
    plt.savefig(dir_path + 'figure/R2_risk_heat_map_{}-p{:.6f}-{}-{}{}.pdf'.format(
        'inpatientVSoutpatient',
        p_val_threshold,
        '-interHRGe1' if interactionge1 else '-interHRNotUsed',
        '-interP{}'.format(interact_p_val_threhold) if interact_p_val_threhold is not None else '-interPNotUsed',
...
if __name__ == '__main__':
    ...
    # 2023-9-28 row in organ order, with organ info, add any pasc
    # df_row = combine_risk_p_value_with_interaction_revision2(database=database, severity='all')
    # df_row = combine_risk_p_value_with_interaction_revision2(database=database, severity='inpatienticu')
    # df_row = combine_risk_p_value_with_interaction_revision2(database=database, severity='outpatient')
