...
if __name__ == "__main__":
    ...
    # %% 1. Load  Data
    # Load PASC information
    df_pasc = pd.read_csv(r'../data/V15_COVID19/output/character/outcome/DX/causal_effects_specific.csv')
    df_pasc_select = df_pasc.sort_values(by='hr-w', ascending=False)
    pvalue = 0.05 / 137
    df_pasc_select = df_pasc_select.loc[df_pasc_select['hr-w-p'] < pvalue, :]  #
    df_pasc_select = df_pasc_select.loc[df_pasc_select['no. pasc in +'] >= 100, :]
    df_pasc_select = df_pasc_select.loc[df_pasc_select['hr-w'] > 1, :]
